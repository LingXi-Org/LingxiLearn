...
async def postgres_repositories():
    ...
    tasks_a = AgentTaskRepository(db_a)
    ledger_a = WorkLedgerRepository(db_a)
    ledger_b = WorkLedgerRepository(db_b)
    learner_id = f"pg-learner-{uuid4().hex}"
    await LearnerRepository(db_a).ensure_learner(learner_id)
    try:
        yield tasks_a, ledger_a, ledger_b, learner_id
    finally:
        await db_a.dispose()
        await db_b.dispose()
...
async def test_postgres_claim_and_budget_reservation_are_cross_instance_atomic(
    ...
    tasks_a, ledger_a, ledger_b, learner_id = postgres_repositories
    task_id = f"pg-task-{uuid4().hex}"
    work_one = f"pg-work-{uuid4().hex}"
    await tasks_a.create_agent_task(
        id=task_id,
    ...
    await ledger_a.append_command(
        task_id=task_id,
        kind="message",
        payload={"message": "开始"},
        idempotency_key="pg-message-1",
    )
    turn = await ledger_a.latest_turn(task_id)
    assert turn is not None
    plan = await ledger_a.create_work_plan(
        task_id=task_id,
        turn_id=str(turn["id"]),
        expected_revision=0,
        items=[
    ...
    claims = await asyncio.gather(
        ledger_a.claim_work_item(work_id=work_one, owner="pg-a"),
        ledger_b.claim_work_item(work_id=work_one, owner="pg-b"),
    )
    assert sum(claim is not None for claim in claims) == 1

    await ledger_b.append_command(
        task_id=task_id,
        kind="message",
        payload={"message": "再次开始"},
        idempotency_key="pg-message-2",
    )
    second_turn = await ledger_b.latest_turn(task_id)
    assert second_turn is not None
    assert second_turn["id"] != turn["id"]
    work_two = f"pg-work-{uuid4().hex}"
    competing = await ledger_b.create_work_plan(
        task_id=task_id,
