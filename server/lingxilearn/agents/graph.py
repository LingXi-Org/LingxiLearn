"""LingxiGraph 2.1.0 coordinator and parallel specialist agents."""
...
2. 按 Skill 要求进行多角度网页研究；使用 web_search 和 web_fetch，不得凭记忆编造事实。
...
def build_agent_graph(
    *,
    ...
    visual_skill = FilesystemSkillSource(REPO_ROOT / "skills" / "visual-explainer")
    lecture_registry = SkillRegistry((lecture_skill,))
    visual_registry = SkillRegistry((visual_skill,))

    async def recognize_intent(state: AgentState, runtime: Runtime[Any]) -> dict[str, Any]:
        _emit(runtime, "intent.started", agent="intent")
        agent = create_agent(model, system_prompt=INTENT_PROMPT, name="intent-recognizer")
    ...
    async def visual_explainer(state: AgentState, runtime: Runtime[Any]) -> dict[str, Any]:
        ...
        async def artifact_write_html(content: str) -> str:
            """Write the task's only allowed visual-explainer HTML artifact."""

            return json.dumps(
                jsonable(artifacts.write_html(state["task_id"], content)), ensure_ascii=False
            )

        async def artifact_validate_html() -> str:
            """Run check_page and light/dark palette validation for the HTML artifact."""

            return json.dumps(
                jsonable(await artifacts.validate_html(state["task_id"])), ensure_ascii=False
            )
    ...
    builder.add_node("lecture_hook", safe_lecture_hook, timeout=settings.agent_timeout)
    builder.add_node("visual_explainer", safe_visual_explainer, timeout=settings.agent_timeout)
