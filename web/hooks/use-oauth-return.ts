...
export function useOAuthReturnRouter() {
  ...
  useEffect(() => {
    ...
    if (ctx.origin === 'workflow') {
      ...
      // Lingxi deliberately has no editable workflow surface. A stale OAuth
      // return context is consumed and lands on the supported Skills page.
      consumeOAuthReturnContext()
      router.replace(`/workspace/${workspaceId}/skills`)
      return
    }
