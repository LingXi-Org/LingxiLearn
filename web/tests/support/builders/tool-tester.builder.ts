...
import {
	createMockFetch as createBaseMockFetch,
 ...
 * Wraps the @/tests/support createMockFetch with tool-specific additions.
 */
...
export class ToolTester<P = unknown, R = unknown> {
	...
	constructor(tool: TestToolConfig<P, R>) {
		...
		this.mockFetch = createToolMockFetch(
			this.mockResponse,
			this.mockResponseOptions,
		);
	...
	setup(
		...
		global.fetch = Object.assign(this.mockFetch, {
			preconnect: vi.fn(),
		}) as typeof fetch;
		return this;
	}
	...
	setupError(errorMessage: string, status = 400) {
		...
		if (status > 0) {
			(this.error as Error & { response: unknown }).response = {
				...
				statusText: errorMessage,
				json: () =>
					Promise.resolve({ error: errorMessage, message: errorMessage }),
			};
		}
	...
	async execute(params: P, _skipProxy = true): Promise<ToolResponse> {
		...
		try {
			let method: string;
			if (
				this.tool.id === "http_request" &&
				(params as Record<string, unknown>)?.method
			) {
	...
	private async handleSuccessfulResponse(
		...
		if (this.tool.id === "http_request") {
			...
			if (
				httpParams.url === "https://api.example.com/data" &&
	...
	getRequestUrl(params: P): string {
		if (this.tool.id === "http_request" && params) {
			...
			if (httpParams.params) {
				const queryParams = httpParams.params as Array<{
					Key: string;
					Value: string;
				}>;
	...
	getRequestHeaders(params: P): Record<string, string> {
		if (this.tool.id === "http_request" && params) {
			...
			if (
				...
				httpParams.method === "GET" &&
				httpParams.headers &&
				(httpParams.headers as Array<{ Key: string; Value: string }>).length ===
					2 &&
				(httpParams.headers as Array<{ Key: string; Value: string }>)[0]
					?.Key === "Authorization"
			) {
				return {
					Authorization: (
						httpParams.headers as Array<{ Key: string; Value: string }>
					)[0].Value,
					Accept: (
						httpParams.headers as Array<{ Key: string; Value: string }>
					)[1].Value,
