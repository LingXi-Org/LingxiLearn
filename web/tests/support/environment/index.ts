...
function toolGuard(
	...
	return () => {
		...
		if (!warned) {
			warned = true;
			console.warn(
				`[@/tests/support] Skipping tests that require ${label}. ${result.hint}`,
			);
		}
		return false;
	};
 ...
 */
export const hasPython3 = toolGuard(
	`python3 >= ${MIN_PYTHON.join(".")}`,
	() => {
		const probe = spawnSync("python3", ["--version"], { encoding: "utf8" });
		const match = /Python (\d+)\.(\d+)/.exec(
		...
		return {
			ok: Boolean(
				found &&
					(found.major > minMajor ||
						(found.major === minMajor && found.minor >= minMinor)),
			),
