...
describe("redis-config mock", () => {
	...
	it("defaults to the Redis-unavailable behavior of the real module", async () => {
		...
		await expect(
			redisConfigMock.closeRedisConnection(),
		).resolves.toBeUndefined();
	});

	it("returns the real connection defaults shape", () => {
		expect(
			redisConfigMock.getRedisConnectionDefaults("redis://localhost:6379"),
		).toEqual({
			keepAlive: 1000,
	...
	it("resetRedisConfigMock restores defaults after overrides", async () => {
		...
		redisConfigMockFns.mockGetConfiguredRedisUrl.mockReturnValue(
		...
		expect(redisConfigMock.getConfiguredRedisUrl()).toBe(
			"redis://localhost:6379",
		);
		expect(redisConfigMock.getRedisClient()).toBe(fakeClient);
		await expect(redisConfigMock.acquireLock("k", "v", 10)).resolves.toBe(
			false,
		);
