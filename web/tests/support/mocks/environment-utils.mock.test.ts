...
describe("environment-utils mock", () => {
	...
	it("defaults model a user with no environment variables", async () => {
		await expect(
			environmentUtilsMock.getEnvironmentVariableKeys("user-1"),
		).resolves.toEqual({
			variableNames: [],
			count: 0,
		});
		await expect(
			environmentUtilsMock.getEffectiveDecryptedEnv("user-1"),
		).resolves.toEqual({});
		await expect(
			environmentUtilsMock.getPersonalAndWorkspaceEnv("user-1"),
		).resolves.toEqual({
			personalEncrypted: {},
			workspaceEncrypted: {},
			personalDecrypted: {},
		...
		await expect(
			environmentUtilsMock.getEffectiveEnvironmentSnapshot("user-1"),
		).resolves.toEqual({
			personalEncrypted: {},
			workspaceEncrypted: {},
			personalDecrypted: {},
		...
		await expect(
			environmentUtilsMock.upsertPersonalEnvVars("user-1", {}),
		).resolves.toEqual({
			added: [],
			updated: [],
		});
