...
function getSocketUrlImpl(): string {
	...
	if (
		browserOrigin &&
		!LOCALHOST_HOSTNAMES_MOCK.has(new URL(browserOrigin).hostname)
	) {
 ...
 * @example
 * ```ts
 * import { urlsMockFns } from '@/tests/support'
 *
...
export function resetUrlsMock(): void {
	...
	urlsMockFns.mockGetInternalApiBaseUrl
		.mockReset()
		.mockImplementation(getInternalApiBaseUrlImpl);
	urlsMockFns.mockEnsureAbsoluteUrl
		.mockReset()
		.mockImplementation(ensureAbsoluteUrlImpl);
	urlsMockFns.mockGetBaseDomain
		.mockReset()
		.mockImplementation(getBaseDomainImpl);
	urlsMockFns.mockGetEmailDomain
		.mockReset()
		.mockImplementation(getEmailDomainImpl);
	urlsMockFns.mockIsLoopbackHostname
		.mockReset()
		.mockImplementation(isLoopbackHostnameImpl);
	urlsMockFns.mockIsNonCanonicalSimHost
		.mockReset()
		.mockImplementation(isNonCanonicalSimHostImpl);
	urlsMockFns.mockParseOriginList
		.mockReset()
		.mockImplementation(parseOriginListImpl);
	urlsMockFns.mockIsLocalhostUrl
		.mockReset()
		.mockImplementation(isLocalhostUrlImpl);
	urlsMockFns.mockGetBrowserOrigin
		.mockReset()
		.mockImplementation(getBrowserOriginImpl);
	urlsMockFns.mockIsSafeHttpUrl
		.mockReset()
		.mockImplementation(isSafeHttpUrlImpl);
	urlsMockFns.mockGetSocketServerUrl
		.mockReset()
		.mockImplementation(getSocketServerUrlImpl);
 ...
 * The app's `vitest.setup.ts` installs this mock globally. Every export of the real module is present.
 *
 * @example
 * ```ts
