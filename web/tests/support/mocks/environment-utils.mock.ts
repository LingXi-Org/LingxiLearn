 ...
 * @example
 * ```ts
 * import { environmentUtilsMockFns } from '@/tests/support'
 *
...
export const environmentUtilsMockFns = {
	...
	mockGetEnvironmentVariableKeys: vi
		.fn()
...
export function resetEnvironmentUtilsMock(): void {
	...
	environmentUtilsMockFns.mockUpsertPersonalEnvVars
		.mockReset()
		.mockResolvedValue({ added: [], updated: [] });
	environmentUtilsMockFns.mockUpsertWorkspaceEnvVars
		.mockReset()
		.mockResolvedValue([]);
	environmentUtilsMockFns.mockGetEffectiveDecryptedEnv
		.mockReset()
		.mockResolvedValue({});
}
