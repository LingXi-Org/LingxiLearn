...
export const mockEnvObject: Record<string, EnvMockValue> = new Proxy(envState, {
	get: (_target, prop) =>
		typeof prop === "string" ? readEnvValue(prop) : undefined,
	...
	has: (target, prop) =>
		typeof prop === "string"
			? Object.hasOwn(target, prop) || prop in process.env
			: false,
	...
	ownKeys: (target) =>
		Array.from(new Set([...Object.keys(target), ...Object.keys(process.env)])),
...
export function envBooleanImpl(
 ...
 */
export function createMockGetEnv(
 ...
 * globally in the app's `vitest.setup.ts`. Every export of the real module is
