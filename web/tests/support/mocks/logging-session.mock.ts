 ...
 * @example
 * ```ts
 * import { loggingSessionMockFns } from '@/tests/support'
 *
...
export const loggingSessionMockFns = {
	...
	mockProjectLiveDisplayText: vi.fn(async (_field: string, value: string) => ({
		value,
	})),
	mockProjectDiagnosticError: vi.fn(
		(error: unknown, _details: Record<string, unknown> = {}) => ({
			errorType:
...
function buildLoggingSessionInstance() {
	return {
		...
		completeWithCancellation:
			loggingSessionMockFns.mockCompleteWithCancellation,
		...
		setTrustedExecutionCorrelation:
			loggingSessionMockFns.mockSetTrustedExecutionCorrelation,
		...
		projectBlockLogsForDisplay:
			loggingSessionMockFns.mockProjectBlockLogsForDisplay,
		...
		safeCompleteWithCancellation:
			loggingSessionMockFns.mockSafeCompleteWithCancellation,
 ...
 */
export const LoggingSessionMock = vi
	.fn()
	.mockImplementation(buildLoggingSessionInstance);
