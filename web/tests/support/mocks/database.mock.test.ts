...
describe("database mock", () => {
	...
	it("resolves empty arrays by default at every terminal", async () => {
		...
		await expect(
			db.select().from(workflowTable).where({}).limit(5),
		).resolves.toEqual([]);
		await expect(
			db.select().from(workflowTable).where({}).orderBy("id"),
		).resolves.toEqual([]);
		await expect(
			db.insert(workflowTable).values({}).returning(),
		).resolves.toEqual([]);
	...
	it("resolves queued rows through downstream terminals (limit/orderBy/joins)", async () => {
		queueTableRows(workflowTable, [{ id: "w-1" }]);
		await expect(
			db.select().from(workflowTable).where({}).limit(1),
	...
	it("supports the .limit(n).for(mode) row-lock chain", async () => {
		...
		await expect(
			db.select().from(workflowTable).where({}).limit(1).for("update"),
	...
	it("routes selectDistinctOn chains through the same table queues", async () => {
		...
		await expect(
			db.selectDistinctOn(["id"]).from(memberTable).where({}),
