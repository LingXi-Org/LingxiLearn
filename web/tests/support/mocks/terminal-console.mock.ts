...
const mockGetWorkflowEntries = vi.fn(
	(workflowId: string) => entriesByWorkflow[workflowId] ?? [],
);

const mockAddConsole = vi.fn((entry: ConsoleEntryLike) => {
	const stored = {
		...entry,
		id: entry.id ?? `mock-${generateRandomString(16)}`,
	};
	if (!entriesByWorkflow[entry.workflowId])
		entriesByWorkflow[entry.workflowId] = [];
...
export function resetTerminalConsoleMock(): void {
	for (const key of Object.keys(entriesByWorkflow))
		delete entriesByWorkflow[key];
