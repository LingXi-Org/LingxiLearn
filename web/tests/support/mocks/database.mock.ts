...
export function createMockSql() {
	...
	sqlFn.join = (fragments: any[], separator: any) => ({
		...
		toSQL: () => ({
			sql: fragments
				.map((f) => f?.toSQL?.()?.sql || String(f))
				.join(separator?.rawSql || ", "),
	...
	// (lib/db/db.ts), which leaves postgres-js with no array serializer, so an
		...
		if (encoder === undefined && Array.isArray(value)) {
			throw new Error(
				"sql.param(array) binds an array as one parameter, which fails under " +
					"fetch_types: false (lib/db/db.ts). Interpolate the array directly " +
...
export function createMockSqlOperators() {
	return {
		...
		notInArray: vi.fn((column, values) => ({
			type: "notInArray",
			column,
			values,
		})),
...
const chainSpy = (): ChainSpy =>
	vi.fn((..._args: any[]) => CHAIN_DEFAULT as any);
...
const onConflictDoUpdate = vi.fn(
	() => ({ returning }) as unknown as Promise<void>,
);
...
const lazyRowsThenable = (getRows: RowsSupplier): any => ({
	then: (
		...
		Promise.resolve((getRows() ?? []) as unknown[]).then(
			onFulfilled,
			onRejected,
		),
...
/**
 * The single shared `@/lib/db` mock instance backing BOTH `dbChainMock` and
...
const dbInstance = {
	...
	update: spyOrDefault(update, () => ({
		set: spyOrDefault(set, mutationWhere),
	})),
...
/**
 * Static mock module for `@/lib/db` backed by `dbChainMockFns`.
 *
 * @example
 * ```ts
 * vi.mock('@/lib/db', () => dbChainMock)
 * ```
 */
...
/**
 * Mock module for `@/lib/db` installed globally in vitest.setup.ts. Shares its
