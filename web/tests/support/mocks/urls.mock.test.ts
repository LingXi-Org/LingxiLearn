...
describe("urls mock", () => {
	...
	it("throws from getBaseUrl when NEXT_PUBLIC_APP_URL is pinned unset", () => {
		...
		expect(() => urlsMock.getBaseUrl()).toThrow(
			"NEXT_PUBLIC_APP_URL must be configured",
		);
	});

	it("getInternalApiBaseUrl prefers INTERNAL_API_BASE_URL and falls back to base URL", () => {
		expect(urlsMock.getInternalApiBaseUrl()).toBe(
			defaultMockEnv.NEXT_PUBLIC_APP_URL,
		);
		setEnv({
			INTERNAL_API_BASE_URL: "http://sim-app.default.svc.cluster.local:3000",
		});
		expect(urlsMock.getInternalApiBaseUrl()).toBe(
			"http://sim-app.default.svc.cluster.local:3000",
		);
	});
	...
	it("pure helpers behave like the real module", () => {
		...
		expect(
			urlsMock.parseOriginList(
				"https://a.example.com/path, https://a.example.com, bad-url",
			),
	...
	it("socket and ollama URLs read env with localhost fallbacks", () => {
		...
		setEnv({
			SOCKET_SERVER_URL: "http://sockets:3002",
