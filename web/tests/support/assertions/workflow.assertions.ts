...
export function expectBlockExists(
	...
	if (expectedType) {
		expect(
			blocks[blockId].type,
			`Block "${blockId}" should be type "${expectedType}"`,
		).toBe(expectedType);
	}
}
 ...
 * ```
 */
export function expectBlockNotExists(
	blocks: Record<string, any>,
	blockId: string,
): void {
	expect(
		blocks[blockId],
		`Block "${blockId}" should not exist`,
	).toBeUndefined();
}
 ...
 * ```
 */
export function expectEdgeConnects(
	edges: any[],
	sourceId: string,
	targetId: string,
): void {
	...
	expect(
		edge,
		`Edge from "${sourceId}" to "${targetId}" should exist`,
	).toBeDefined();
}
...
export function expectNoEdgeBetween(
	...
	expect(
		edge,
		`Edge from "${sourceId}" to "${targetId}" should not exist`,
	).toBeUndefined();
}
...
export function expectBlockHasParent(
	...
	expect(
		block.data?.parentId,
		`Block "${childId}" should have parent "${expectedParentId}"`,
...
export function expectBlockCount(workflow: any, expectedCount: number): void {
	...
	expect(actualCount, `Workflow should have ${expectedCount} blocks`).toBe(
		expectedCount,
	);
}
...
export function expectEdgeCount(workflow: any, expectedCount: number): void {
	expect(
		...
		`Workflow should have ${expectedCount} edges`,
	).toBe(expectedCount);
}
...
export function expectBlockPosition(
	...
	expect(block.position.x, `Block "${blockId}" x position`).toBeCloseTo(
		expectedPosition.x,
		0,
	);
	expect(block.position.y, `Block "${blockId}" y position`).toBeCloseTo(
		expectedPosition.y,
		0,
	);
}
 ...
 * ```
 */
export function expectBlockDisabled(
	blocks: Record<string, any>,
	blockId: string,
): void {
	const block = blocks[blockId];
...
export function expectEmptyWorkflow(workflow: any): void {
	expect(
		Object.keys(workflow.blocks).length,
		"Workflow should have no blocks",
	).toBe(0);
	expect(workflow.edges.length, "Workflow should have no edges").toBe(0);
	expect(
		Object.keys(workflow.loops).length,
		"Workflow should have no loops",
	).toBe(0);
	expect(
		Object.keys(workflow.parallels).length,
		"Workflow should have no parallels",
	).toBe(0);
}
