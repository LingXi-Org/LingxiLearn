 ...
 * ```
 */
export function expectBlockExecuted(
	ctx: ExecutionContext,
	blockId: string,
): void {
	expect(
		ctx.executedBlocks.has(blockId),
		`Block "${blockId}" should have been executed`,
	).toBe(true);
}
 ...
 * ```
 */
export function expectBlockNotExecuted(
	ctx: ExecutionContext,
	blockId: string,
): void {
	expect(
		ctx.executedBlocks.has(blockId),
		`Block "${blockId}" should not have been executed`,
	).toBe(false);
}
 ...
 * ```
 */
export function expectExecutionOrder(
	executedBlocks: string[],
	expectedOrder: string[],
): void {
	const actualOrder = executedBlocks.filter((id) => expectedOrder.includes(id));
	expect(actualOrder, "Blocks should be executed in expected order").toEqual(
		expectedOrder,
	);
}
 ...
 * ```
 */
export function expectLogCount(
	ctx: ExecutionContext,
	expectedCount: number,
): void {
	expect(ctx.blockLogs.length, `Should have ${expectedCount} logs`).toBe(
		expectedCount,
	);
}
...
export function expectConditionDecision(
	...
	expect(
		decision,
		`Condition "${blockId}" should have a decision`,
	).toBeDefined();
 ...
 * ```
 */
export function expectLoopCompleted(
	ctx: ExecutionContext,
	loopId: string,
): void {
	expect(
		ctx.completedLoops.has(loopId),
		`Loop "${loopId}" should be completed`,
	).toBe(true);
}
 ...
 * ```
 */
export function expectInActivePath(
	ctx: ExecutionContext,
	blockId: string,
): void {
	expect(
		ctx.activeExecutionPath.has(blockId),
		`Block "${blockId}" should be in active path`,
	).toBe(true);
}
