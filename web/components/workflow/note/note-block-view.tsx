...
import { ChevronsDownUp, Expand } from "@/components/ui-kit/icons";
...
const NOTE_COMPONENTS = {
	p: ({ children }: { children?: ReactNode }) => (
		<p className="mb-1 break-words text-current text-sm leading-[1.25rem] last:mb-0">
			{children}
		</p>
	),
	...
	input: ({ type, checked }: ComponentProps<"input">) =>
		type === "checkbox" ? (
			<input
				...
				checked={checked}
				disabled
				className={NOTE_TASK_CHECKBOX_CLASS}
			/>
		) : null,
	/**
	...
	strong: ({ children }: { children?: ReactNode }) => (
		<strong className="break-words font-semibold text-current">
			{children}
		</strong>
	),
	...
	th: ({ children }: { children?: ReactNode }) => (
		<th className="px-2 py-1 text-left font-semibold text-current">
			{children}
		</th>
	),
 ...
 */
export const NOTE_MARKDOWN_FLOW =
...
export function NoteBlockView({
	...
	const [compactHeight, setCompactHeight] = useState(() =>
		estimateNoteBlockHeight(content),
	);
	...
	const blockWidth = isExpanded
		? BLOCK_DIMENSIONS.NOTE_EXPANDED_WIDTH
		: BLOCK_DIMENSIONS.NOTE_WIDTH;
	const blockHeight = isExpanded
		? BLOCK_DIMENSIONS.NOTE_EXPANDED_HEIGHT
		: compactHeight;
	/*
	...
	const measureBlockHeight = useCallback(() => {
		...
		const measuredContentHeight = isEmpty
			? 0
	...
	useEffect(() => {
		if (isExpanded || editingField === "content" || !contentMeasureRef.current)
			return;
				...
				? [
						{
							...
							side: "top",
							position: {
								fromEnd: ACTION_MENU_RIGHT_INSET_PX + actionMenuWidth / 2,
							},
             ...
             hover. */
               ...
               saying "this is the thing you are acting on". */
						...
						silhouetteColorOverride={
							!hasVisualFocus && isHovered
								? colorOption.hoverSilhouetteColor
								: undefined
						}
						...
						<div className="flex min-w-0 flex-1 items-center transition-opacity duration-150 [transition-timing-function:cubic-bezier(0.23,1,0.32,1)]">
							{editingField === "title" ? (
								...
								<button
									...
									onPointerDown={(event) =>
										recordEditPointerStart(event, "title")
									}
								...
								>
									<OverflowSpan
										value={name ?? ""}
										className="truncate text-[17px] text-current"
									/>
								</button>
							) : (
								<OverflowSpan
									value={name ?? ""}
									className={cn(
										"truncate text-[17px] text-current",
										!isEnabled && "opacity-50",
									)}
								/>
							)}
							...
							<>
								{isInlineEditable && (
									<button
										...
										onPointerDown={(event) =>
											recordEditPointerStart(event, "content")
										}
								...
								>
									{isEmpty ? (
										<p
											className={cn(
												"text-sm",
												colorOption.placeholderClassName,
											)}
										>
											Add note…
										</p>
