...
import { Ban, Lock, Repeat, Split } from "@/components/ui-kit/icons";
...
export function SubflowStartView({
	...
	const startHandleId =
		kind === "loop" ? "loop-start-source" : "parallel-start-source";
	/*
	...
	const [cursorSourceHandle, setCursorSourceHandle] =
		useState<WorkflowBorderCursorHandle | null>(null);
	...
	const syncCursorSourceHandleBounds = useCallback(() => {
		...
		const sourceBounds =
			state.nodeInternals.get(parentId)?.[internalsSymbol]?.handleBounds
				?.source;
	...
	return (
		...
		>
			...
			/>
			<span className="relative z-10 text-[var(--text-primary)] text-sm">
				Start
			</span>
			<Handle
				type="source"
...
export function SubflowNodeView({
	...
	const cursorSourceHandleKeyRef = useRef<string | null>(null);
	const [cursorSourceHandle, setCursorSourceHandle] =
		useState<SubflowCursorSourceHandle | null>(null);
	const isPreview = data?.isPreview || false;
	const isPreviewSelected = data?.isPreviewSelected || false;

	const endHandleId =
		data.kind === "loop" ? "loop-end-source" : "parallel-end-source";
			...
			>
				...
				>
					...
					<div className="relative z-10 flex flex-shrink-0 items-center gap-1">
						{!isEnabled && (
							<SubflowStateIndicator label="Disabled" Icon={Ban} />
						)}
