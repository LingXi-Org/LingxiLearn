...
import { usePrefersReducedMotion } from "@/components/ui-kit";
...
export function WorkflowEdgeView({
	...
	const isTraversing =
		isWorkflowRunning && hasRunStatus && isTargetActive && !diffStatus;
	/*
							...
							return (
								<path
									...
									data-workflow-edge-pulse-glow={
										layer.id === "tail" ? "" : undefined
									}
									data-workflow-edge-traversing={
										layer.id === "core" ? "" : undefined
									}
									d={edgePath}
									fill="none"
									stroke="var(--white)"
									...
									filter={
										layer.id === "tail" ? `url(#${pulseGlowId})` : undefined
									}
