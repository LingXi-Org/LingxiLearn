...
function renderEdge(overrides: Partial<WorkflowEdgeViewProps> = {}) {
	...
	return {
		...
		pulseLayers: host.querySelectorAll<SVGPathElement>(
			"[data-workflow-edge-pulse-layer]",
		),
		traversingPath: host.querySelector<SVGPathElement>(
			"[data-workflow-edge-traversing]",
		),
		pulseGlow: host.querySelector<SVGPathElement>(
			"[data-workflow-edge-pulse-glow]",
		),
	};
}
...
describe("WorkflowEdgeView", () => {
	...
	it("mutes edges that have not been taken during execution", () => {
		...
		expect(
			host.querySelector('[data-workflow-edge-state="waiting"]'),
		).not.toBeNull();
	});
	...
	it("moves a signal toward the currently active target", () => {
		...
		expect(
			host.querySelector('[data-workflow-edge-state="traversing"]'),
		).not.toBeNull();
		...
		expect(
			traversingPath?.querySelector("animate")?.getAttribute("repeatCount"),
		).toBe("indefinite");
		expect(traversingPath?.querySelector("animate")?.getAttribute("from")).toBe(
			"-0.125",
		);
		expect(traversingPath?.querySelector("animate")?.getAttribute("to")).toBe(
			"-2.325",
		);
		expect(traversingPath?.querySelector("animate")?.getAttribute("dur")).toBe(
			"1100ms",
		);
		expect(
	...
	it("settles a taken edge when its target is no longer active", () => {
		...
		expect(
			host.querySelector('[data-workflow-edge-state="traversed"]'),
		).not.toBeNull();
		expect(traversingPath).toBeNull();
	});
