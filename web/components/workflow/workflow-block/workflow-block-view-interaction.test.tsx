...
describe("WorkflowBlockView action menu", () => {
	...
	it("places active block-state indicators before the type tag", () => {
		...
		const typeTag = host.querySelector(
			'[data-workflow-type-accent="function"]',
		);
...
describe("WorkflowTypeTag colors", () => {
	it("maps core blocks through semantic workflow roles", () => {
		...
		expect(getWorkflowTypeAccent("agent")).toEqual({
			variant: "workflow",
			tone: "inverse",
		});
		expect(getWorkflowTypeAccent("api")).toEqual({
			variant: "workflow",
			tone: "blue",
		});
		expect(getWorkflowTypeAccent("loop")).toEqual({
			variant: "workflow",
			tone: "ash",
		});
		expect(getWorkflowTypeAccent("parallel")).toEqual({
			variant: "workflow",
			tone: "ash",
		});
		expect(getWorkflowTypeAccent("router")).toEqual({
			variant: "workflow",
			tone: "ash",
		});
		...
		expect(getWorkflowTypeAccent("knowledge")).toEqual({
			variant: "workflow",
			tone: "content",
		});
	});
	...
	it("renders compact workflow icons with their canonical fill and ink", () => {
		...
		expect(
			host.querySelector('[data-workflow-type-icon="knowledge"]'),
		).toHaveClass("bg-[#007E80]", "text-[#FFFFFF]");
		expect(
			host.querySelector('[data-workflow-type-icon="image_generator_v2"]'),
		).toHaveClass("bg-[#AA00FF]", "text-[#F8F8F8]");
	});
	...
	it("uses the provider background with a contrasting shared icon and label color", () => {
		...
		const lightTag = host.querySelector<HTMLElement>(
	...
	it("names the block type even when the title already says it", () => {
		...
		expect(
			host.querySelector<HTMLElement>("[data-workflow-type-accent]"),
