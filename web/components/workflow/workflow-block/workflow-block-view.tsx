...
import { Ban, Lock } from "@/components/ui-kit/icons";
import {
...
const ACTION_MENU_MAX_WIDTH_PX =
	BLOCK_DIMENSIONS.FIXED_WIDTH - ACTION_MENU_RIGHT_INSET_PX * 2;
...
export function getNearestBranchCursorHandleId(
	...
	const nearestIndex = Math.min(
		...
		Math.max(
			0,
			Math.round((cursorY - firstRowY) / HANDLE_POSITIONS.CONDITION_ROW_HEIGHT),
		),
	);
	return getCursorBranchSourceHandleId(
		`${handlePrefix}-${rows[nearestIndex].id}`,
	);
}
...
export const hasWorkflowTypeRole = (
	type: string,
): type is keyof typeof WORKFLOW_TYPE_ROLES =>
	Object.hasOwn(WORKFLOW_TYPE_ROLES, type);

export const getWorkflowTypeRole = (type: string): WorkflowTypeRole =>
	WORKFLOW_TYPE_ROLES[type as keyof typeof WORKFLOW_TYPE_ROLES] ??
	DEFAULT_WORKFLOW_TYPE_ROLE;

export const getWorkflowTypeAccent = (type: string) =>
	WORKFLOW_ROLE_ACCENTS[getWorkflowTypeRole(type)];

export interface WorkflowTypeIconProps
 ...
 * assertion above.
 */
const getInvisibleHandleClasses = (
...
export function WorkflowBlockView({
	...
	const [cursorSourceHandle, setCursorSourceHandle] =
		useState<WorkflowCursorSourceHandle | null>(null);
	...
	const syncCursorSourceHandleBounds = useCallback(() => {
		const handleElement = cursorSourceHandleRef.current;
		const nodeElement =
			handleElement?.closest<HTMLDivElement>(".react-flow__node") ?? null;
		if (!handleElement || !nodeElement) return;

		const state = reactFlowStore.getState();
		const sourceBounds =
			state.nodeInternals.get(id)?.[internalsSymbol]?.handleBounds?.source;
		const handleId = handleElement.dataset.handleid;
		const handlePosition = handleElement.dataset.handlepos as
			| Position
			| undefined;
		...
		const nextBounds = {
			...
			x:
				(handleBounds.left - nodeBounds.left - nodeBounds.width * originX) /
				zoom,
			y:
				(handleBounds.top - nodeBounds.top - nodeBounds.height * originY) /
				zoom,
	...
	const showErrorRow =
		showErrorControls && shouldShowDefaultHandles && type !== "response";
	 ...
	 */
	const rendersErrorHandle =
		showErrorRow && (errorOutputEnabled || hasErrorConnection);
	useEffect(() => {
		if (supportsCursorHandle) return;
		cursorSourceHandleKeyRef.current = null;
		setCursorSourceHandle(null);
	...
	const branchRowCount =
		type === "condition" ? conditionRows.length : routerRows.length;
	const rowTabLength = clampTabLength(
		branchRowCount <= 2
			? TAB_LENGTH_SMALL_PX
			: TAB_LENGTH_SMALL_PX - (branchRowCount - 2) * 2,
	);
		...
		if (type === "condition") {
			conditionRows.forEach((condition, index) => {
				ports.push({
					...
					position:
						HANDLE_POSITIONS.CONDITION_START_Y +
						index * HANDLE_POSITIONS.CONDITION_ROW_HEIGHT,
			...
			>
				<WorkflowBlockBorder
					...
					onCursorHandleChange={
						supportsCursorHandle ? onCursorHandleChange : undefined
					}
				...
				{cursorSourceHandle && (
					<Handle
						...
						type="source"
						position={getCursorSourceHandlePosition(
							cursorSourceHandle.edgeSide,
						)}
						...
						isValidConnection={(connection) => {
							...
							return !wouldCreateConnectionCycle(
								connection.source!,
								connection.target!,
							);
						}}
             ...
             tag against the painted silhouette. */
					style={
						!hasContentBelowHeader
							? { height: BLOCK_DIMENSIONS.MIN_PAINTED_HEIGHT }
							: undefined
					}
				>
					<div
							...
							<Tooltip.Root>
								<Tooltip.Trigger asChild>
									<Badge
										...
										className={
											canFixSunset ? "cursor-pointer" : "cursor-not-allowed"
										}
										dot
								...
								<Tooltip.Root>
									<Tooltip.Trigger asChild>
										<Badge
											...
											className={
												canAdmin ? "cursor-pointer" : "cursor-not-allowed"
											}
											dot
											onClick={(e) => {
												e.stopPropagation();
						...
						{type === "condition" ? (
							...
							<>
								<SubBlockRowView
									key="context"
									title="Context"
									displayValue={routerContextValue}
								/>
