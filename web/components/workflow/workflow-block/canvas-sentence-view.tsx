 ...
 * `apps/*`.
 */
export type CanvasSentenceSegment =
	| string
	| { subBlockId: string; noun?: string };
 ...
 */
export function CanvasSentenceView({
	segments,
	renderChip,
}: CanvasSentenceViewProps) {
	return (
		<p className="min-w-0 break-words text-[var(--text-muted)] text-sm leading-6">
			{segments.map((segment, index) => {
				if (typeof segment === "string") {
					...
					return (
						<Fragment key={`text-${index}`}>{`${glue}${segment}`}</Fragment>
					);
				}
