...
describe("WorkflowBlockBorder mount", () => {
	it("resolves every connection start to the one canonical source handle", () => {
       ...
       edit pipeline recognizes any id but `source`. */
		...
		expect(normalizeCursorSourceHandleId("source-cursor-bottom")).toBe(
			"source",
		);
		expect(normalizeCursorSourceHandleId("source-cursor-left", "loop")).toBe(
			"loop-end-source",
		);
		expect(
			normalizeCursorSourceHandleId("source-cursor-right", "parallel"),
		).toBe("parallel-end-source");
	});
	...
	it("keeps a subflow Start drag anchored to the start source, not the container exit", () => {
       ...
       otherwise persist as an edge leaving the container. */
		expect(
			normalizeCursorSourceHandleId(
				getCursorBranchSourceHandleId("loop-start-source"),
				"loop",
			),
	...
	it("keeps moving branch swells attached to a valid branch output", () => {
		...
		expect(
			getNearestBranchCursorHandleId(conditionRows, 0, 60, "condition"),
		).toBe(getCursorBranchSourceHandleId("condition-if-id"));
		expect(
			getNearestBranchCursorHandleId(conditionRows, 90, 60, "condition"),
		).toBe(getCursorBranchSourceHandleId("condition-else-if-id"));
		expect(
			getNearestBranchCursorHandleId(conditionRows, 200, 60, "condition"),
		).toBe(getCursorBranchSourceHandleId("condition-else-id"));
		expect(
			normalizeCursorSourceHandleId(
				getCursorBranchSourceHandleId("condition-else-id"),
			),
		).toBe("condition-else-id");
	});
	...
	it("heals side-anchored handle ids back onto the canonical pair", () => {
		for (const side of ["top", "right", "bottom", "left"]) {
			expect(normalizeWorkflowEdgeSourceHandle(`source-${side}`)).toBe(
				"source",
			);
			expect(normalizeWorkflowEdgeTargetHandle(`target-${side}`)).toBe(
				"target",
			);
		}
       ...
       duplicate check keeps matching what persistence actually writes. */
		expect(normalizeWorkflowEdgeSourceHandle("condition-if-id")).toBe(
			"condition-if-id",
		);
		expect(normalizeWorkflowEdgeSourceHandle("loop-end-source")).toBe(
			"loop-end-source",
		);
	...
	it("paints camera-followed execution with the selected silhouette color", () => {
		...
		expect(
			host.querySelector('path[fill="var(--text-secondary)"]'),
		).toBeTruthy();
		expect(
			host.querySelector('path[stroke="var(--border-success)"]'),
		).toBeNull();
	});
	...
	it("mounts a header-only card without throwing", () => {
		const { host } = mount(
			<div style={{ width: 250, height: 48 }}>
				<WorkflowBlockBorder
					ports={[
						{ id: "source", side: "right", position: "center", plateau: 10 },
					]}
					hasRing={false}
					ringStyles=""
					height={48}
	...
	it("bounds note content in a faded scroll viewport without connection handles", () => {
		...
		expect(scrollRegion).not.toHaveClass(
			"pointer-events-none",
			"overflow-hidden",
		);
		expect(scrollRegion).not.toHaveClass("allow-scroll", "nodrag", "nopan");
		expect(host.querySelector("[data-note-scroll-region] > div")).toHaveClass(
			"pointer-events-none",
		);
		...
		expect(
			host.querySelector("[data-workflow-action-bar-bridge]"),
		).toBeTruthy();
		expect(host.querySelector('svg rect[fill="#525252"]')).toBeTruthy();
		expect(
			host.querySelector(
				'svg path[fill="var(--border-1)"][stroke="var(--border-1)"]',
			),
		).toBeTruthy();
		...
		expect(host.querySelector("[data-note-scroll-region]")).not.toHaveClass(
			"nodrag",
			"nopan",
		);
		expect(host.querySelector("[data-note-scroll-region]")).not.toHaveClass(
		...
		expect(
			host.querySelector('textarea[aria-label="Note content"]'),
		).toBeNull();
		expect(host.querySelector("svg")).toHaveAttribute(
			"viewBox",
			"-36 -36 392 312",
		);
		...
		expect(
			host.querySelector('svg path[fill="var(--text-secondary)"]'),
		).toBeTruthy();
		expect(
			host.querySelector('button[aria-label="Edit note title"]'),
		).toBeNull();
		expect(
			host.querySelector('button[aria-label="Edit note content"]'),
		).toBeNull();
		...
		expect(host.querySelector("[data-note-card]")?.contains(expandButton)).toBe(
			true,
		);
		...
		expect(host.querySelector("[data-node-selected]")).toBeTruthy();
		expect(
			host.querySelector('button[aria-label="Collapse note"]'),
		).toBeTruthy();
		...
		act(() => {
			contentEditTrigger?.dispatchEvent(
       ...
       selection tint reaches the editor as a prop rather than a class override. */
		expect(contentInput?.parentElement).toHaveClass(
			"nodrag",
			"nopan",
			"nowheel",
		);
		...
		expect(handleContentChange).toHaveBeenCalledWith(
			expect.stringContaining("Maximum height"),
		);
		expect(host.querySelector("[data-note-card]")).toHaveStyle({
			height: "400px",
		});
       ...
       plain value surface with no exit callback of its own. */
		...
		expect(host.querySelector('svg rect[fill="#525252"]')).toBeTruthy();
		expect(
			host.querySelector('svg path[fill="#3F3F3F"][stroke="#3F3F3F"]'),
		).toBeTruthy();
	...
	it("keeps an expanded note anchored while its compact height grows", () => {
		...
		expect(host.querySelector("[data-note-layout]")).toHaveStyle({
			height: "70px",
		});
		...
		act(() => {
			...
			setValue?.call(
				contentInput,
				"Content that increases the compact note height",
			);
			contentInput?.dispatchEvent(new Event("input", { bubbles: true }));
		});
		 ...
		 */
		expect(
			handleHeightChange.mock.calls.every(([height]) => height === 70),
		).toBe(true);
		expect(host.querySelector("[data-note-layout]")).toHaveStyle({
			height: "70px",
		});
	...
	it("never publishes a height measured at the expanded width", () => {
		...
		const measureNode = host.querySelector<HTMLElement>(
			"[data-note-scroll-region] > div",
		);
		expect(measureNode).toBeTruthy();
		Object.defineProperty(measureNode, "scrollHeight", {
			configurable: true,
			value: 300,
		});
	...
	it("re-measures after a collapse that never blurred the editor", () => {
		...
		const { host, root } = mount(
			<NoteBlockView {...noteProps} isFocused isExpanded />,
		);
		...
		act(() =>
			root.render(
				<NoteBlockView {...noteProps} isFocused={false} canEdit={false} />,
			),
		);
		...
		expect(handleHeightChange).toHaveBeenLastCalledWith(
			getNoteBlockHeight(false),
		);
	});
	...
	it("paints the subflow Start source as a unified connection swell", () => {
		...
		expect(start?.querySelector("svg")).toHaveAttribute(
			"viewBox",
			"-36 -36 130 106",
		);
		expect(
			start?.querySelector('[data-handleid="loop-start-source"]'),
		).toBeTruthy();
		expect(
			start?.querySelector('path[stroke="var(--text-secondary)"]'),
		).toBeTruthy();
	});
	...
	it("uses the standard card header and lower full-size ports for subflows", () => {
		...
		expect(
			host.querySelector('[data-subflow-type-tag="loop"]'),
		).toHaveTextContent("Loop");
		expect(host.querySelector('[data-handleid="target"]')).toHaveStyle({
			top: "69px",
		});
		expect(host.querySelector('[data-handleid="loop-end-source"]')).toHaveStyle(
			{ top: "69px" },
		);
		expect(host.querySelector("svg")).toHaveAttribute(
			"viewBox",
		...
		expect(
			host.querySelector('svg rect[fill="var(--surface-3)"]'),
		).toBeTruthy();
		const dropTargetOutline = host.querySelector(
	...
	it("gives a nested block exclusive hover ownership over the loop action swell", () => {
				...
				<div>
					<div className="react-flow__node" data-loop-node="loop-1">
						<SubflowNodeView
							...
							onSelect={() => undefined}
							actionBar={
								<div data-workflow-action-bar-swell="">Loop actions</div>
							}
						/>
					</div>
		...
		const actionMenuRoot = host.querySelector(
			'[data-node-id="loop-1"]',
		)?.parentElement;
		...
		act(() => loopNode?.dispatchEvent(new Event("pointerenter")));
		act(() =>
			loopHeader?.dispatchEvent(new Event("pointerover", { bubbles: true })),
		);
		expect(actionMenuRoot).toHaveAttribute("data-action-menu-open");
	});
	...
	it("keeps every path coordinate bounded when animation timestamps move backward", () => {
		...
		const pathData = [...host.querySelectorAll("svg path")]
			...
			.join(" ");
		const coordinates =
			pathData.match(/-?(?:\d+\.?\d*|\.\d+)(?:e[+-]?\d+)?/gi)?.map(Number) ??
			[];
