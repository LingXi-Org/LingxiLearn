    ...
    <div className="sim-workspace-window-frame relative flex h-dvh min-h-0 w-full bg-[var(--bg)]">
      ...
      <div className="flex min-w-0 flex-1 flex-col p-2 pl-0">
        <div className="flex h-11 shrink-0 items-center border-b border-[var(--border)] bg-[var(--surface-1)] px-3 md:hidden">
          <SimButton type="button" variant="quiet" size="icon" onClick={() => setMobileSidebarOpen(true)} className="grid size-8 place-items-center rounded-lg" aria-label="打开侧栏"><Menu className="size-[18px]" /></SimButton>
      ...
      {mobileSidebarOpen && <div className="fixed inset-0 z-50 flex bg-black/20 md:hidden" onClick={() => setMobileSidebarOpen(false)}><div className="relative h-full w-[238px] shadow-[var(--shadow-overlay)]" onClick={(event) => event.stopPropagation()}><SimButton type="button" variant="quiet" size="icon" onClick={() => setMobileSidebarOpen(false)} className="absolute left-[246px] top-3 z-10 grid size-8 place-items-center rounded-lg bg-[var(--surface-1)] shadow-sm" aria-label="关闭侧栏"><X className="size-4" /></SimButton><SimSidebar {...sidebar} collapsed={false} onClose={() => setMobileSidebarOpen(false)} /></div></div>}
    </div>
  );
}
