...
const UserInputImpl = forwardRef<UserInputHandle, UserInputProps>(function UserInput(
  ...
  const handleFileSelectStable = useCallback(() => {
    toast.info('未接入：LingxiGraph 当前不支持附件上传')
  }, [])
  ...
  const handlePlusClick = useCallback((e: React.MouseEvent<HTMLButtonElement>) => {
    e.preventDefault()
    toast.info('未接入：文件、表格、知识库和工作流资源尚未连接')
  }, [])
      ...
      <div className='flex items-center justify-between'>
        <div className='flex items-center gap-1'>
          <Tooltip.Root>
            ...
            <Tooltip.Content side='top'>添加资源 · 未接入</Tooltip.Content>
          </Tooltip.Root>
          <Tooltip.Root>
            <Tooltip.Trigger asChild>
              <Button
            ...
            <Tooltip.Content side='top'>附件 · 未接入</Tooltip.Content>
          </Tooltip.Root>
          <Tooltip.Root>
            <Tooltip.Trigger asChild>
              <Button
        ...
        <div className='flex items-center gap-1.5'>
          {isSttSupported && (
            <MicButton
              isListening={false}
              onToggle={() => toast.info('未接入：语音输入尚未连接 LingxiGraph')}
            />
          )}
          <SendButton
            isSending={isSending}
