...
function LogDropdownItem({ item }: DropdownItemRenderProps) {
  const activityName = (item.activityName as string) ?? item.name
...
export const RESOURCE_REGISTRY: Partial<Record<MothershipResourceType, ResourceTypeConfig>> & { generic: ResourceTypeConfig } = {
  generic: {
    type: 'generic',
    label: 'Results',
    icon: TerminalWindow,
...
export function getResourceConfig(type: MothershipResourceType): ResourceTypeConfig {
  return RESOURCE_REGISTRY[type] ?? RESOURCE_REGISTRY.generic
}

type CacheableResourceType = Exclude<MothershipResourceType, 'generic' | 'workflow'>
...
export function invalidateResourceQueries(
  ...
  if (resourceType === 'generic' || resourceType === 'workflow') return
