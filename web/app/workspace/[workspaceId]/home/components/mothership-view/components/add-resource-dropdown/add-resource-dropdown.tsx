 ...
 * legacy (`folder`) and workspace-file (`filefolder`) folders these are not
...
interface UseAvailableResourcesOptions {
   ...
   *
   * Note this only defers the lists; the mothership tab bar independently
   * resolves names for the supported table, file, knowledge-base, and folder
   * lists, so those stay warm there.
   */
  enabled?: boolean
  /**
...
export function useAvailableResources(
  ...
  const groups = useMemo(() => {
    ...
    const groups: AvailableItemsByType[] = [
      ...
      },
      {
        type: 'log' as const,
        items: logs.map((log) => {
          const activityName = log.workflow?.name ?? log.workflowId ?? 'Agent activity'
          const time = formatDate(log.createdAt).compact
          return { id: log.id, name: `${activityName} · ${time}`, activityName, time }
        }),
      },
    ]
   ...
   * folders carry no user-defined ordering beyond their hierarchy.
   */
...
interface ResourceFolderTreeItemsProps {
   ...
   * folders that only provide structure (tables and knowledge bases).
   */
...
interface FolderedSectionSpec {
   ...
   */
  folderType?: MothershipResourceType
  /** Interleave folders and items by `sortOrder` where a supported family uses it. */
