...
export interface BuildResourceFolderTreeOptions {
  /**
   * Order each level by `sortOrder` when supplied by a resource family.
 ...
 * Shared by every foldered resource family (files, tables, knowledge bases);
 * the families differ only in the options above and in whether the
