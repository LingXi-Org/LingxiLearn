  ...
  return (
    <main className="min-h-screen bg-neutral-950 p-6 font-mono text-xs text-neutral-200">
      <div className="mx-auto max-w-7xl space-y-4">
        ...
        {error ? <pre className="rounded border border-red-900 bg-red-950/40 p-3 text-red-300">{error}</pre> : null}

        <section className="grid gap-4 md:grid-cols-4">
          {[
            ['任务状态', task?.status ?? 'loading'],
            ['运行状态', task?.runtime_status ?? '—'],
          ...
          <div className="max-h-[32rem] overflow-auto">
            <table className="w-full border-collapse text-left">
              <thead className="sticky top-0 bg-neutral-900 text-neutral-500">
                <tr><th className="p-2">#</th><th className="p-2">kind</th><th className="p-2">agent</th><th className="p-2">payload</th></tr>
              </thead>
              <tbody>
                {events.map((event) => (
                  <tr key={`${event.sequence}-${event.kind}`} className="border-t border-neutral-800 align-top">
                    ...
                    <td className="whitespace-pre-wrap break-all p-2 text-neutral-300">{JSON.stringify(event.payload)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <details className="rounded border border-neutral-800 bg-neutral-900 p-4">
          <summary className="cursor-pointer text-sm font-semibold text-white">编排决策 JSON</summary>
