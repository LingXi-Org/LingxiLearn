...
/**
 * LingxiGraph does not expose Sim's workspace ACL endpoint. The port keeps the
 * original consumers and supplies a read-only capability boundary locally.
 */
export function LingxiWorkspacePermissionsProvider({ children }: WorkspacePermissionsProviderProps) {
  const contextValue = useMemo<WorkspacePermissionsContextType>(
    () => ({
      workspacePermissions: null,
      permissionsLoading: false,
      permissionsError: null,
