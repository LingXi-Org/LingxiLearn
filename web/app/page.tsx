...
export default function Home() {
  ...
  return (
    <SimAppShell
      ...
      loading={loading}
      starting={starting}
      onStartMission={(missionId, packId) => void startMission(missionId, packId)}
    >
        ...
        <div className="min-h-0 flex-1 overflow-y-auto">
          <div className="mx-auto flex min-h-full w-full max-w-[1120px] flex-col justify-end px-4 pb-4 pt-12 sm:px-8 sm:pb-8">
            <div className="pb-7 text-center sm:pb-9">
              <h1 data-testid="home-greeting" className="text-[25px] font-medium tracking-[-0.035em] text-[var(--text-primary)] sm:text-[32px]">
                欢迎回来，今天想完成什么？
              </h1>
            ...
            <div className="grid gap-3 pb-4 sm:grid-cols-2 lg:grid-cols-4">
              {QUICK_ACTIONS.map(({ title, icon: Icon, tone }) => (
                <div
                  key={title}
                  className="flex min-h-[132px] flex-col justify-between rounded-[17px] border border-[#dedede] bg-white p-4 transition-colors hover:border-[#c9c9c9] hover:bg-[#fdfdfd] sm:p-5"
                  aria-label={title}
                >
                  <span className={`grid size-9 place-items-center rounded-xl ${tone}`}>
                    <Icon className="size-[18px]" strokeWidth={1.7} />
                  </span>
                  <span className="max-w-[210px] text-left text-[14px] font-medium leading-6 text-[var(--text-primary)] sm:text-[15px]">
                    {title}
                  </span>
                </div>
              ))}
            ...
            <div className="w-full">
              {error && (
                <div role="alert" className="mb-3 flex items-start gap-2 border border-red-200 bg-red-50 px-3 py-2.5 text-[12px] text-red-700">
                  <CircleAlert className="mt-0.5 size-3.5 shrink-0" />
                  <span>课程目录暂时无法加载，请检查后端服务。</span>
                </div>
              )}
              <LearningPrompt
                className="max-w-none"
                onSend={openDraft}
                placeholder="输入你的问题或学习目标…"
              />
            </div>
          </div>
        </div>
