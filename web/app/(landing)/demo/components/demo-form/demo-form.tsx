...
import { Chip, ChipDropdown, ChipInput, ChipTextarea, Label } from '@/components/ui-kit'

const DEMO_REQUEST_COMPANY_SIZE_OPTIONS = [
...
export function DemoForm({ onComplete }: DemoFormProps) {
  ...
  const emailIsValid = emailFormatValid
  const canSubmit =
    emailIsValid &&
  ...
  /**
   * Surface an error only once the value looks like an address attempt.
   */
