...
const CODE_LINES: CodeSegment[][] = [
	...
	[
		{ text: "    " },
		{ text: "'Answer customer questions'", tone: "primary" },
		{ text: "," },
	],
	[
		{ text: "    " },
...
const CODE_LINE_STARTS = CODE_LINE_LENGTHS.map((_, index) =>
	CODE_LINE_LENGTHS.slice(0, index).reduce(
		(total, length) => total + length,
		0,
	),
);
const CODE_TOTAL_CHARS = CODE_LINE_LENGTHS.reduce(
	(total, length) => total + length,
	0,
);
...
export function BuildMethodsGraphic() {
	...
	useEffect(() => {
		...
		const startCodeTyping = () => {
			...
			const interval = setInterval(() => {
				...
				const next = Math.min(
					Math.floor(elapsed / CODE_CHAR_MS) + 1,
					CODE_TOTAL_CHARS,
				);
		...
		const startPromptTyping = () => {
			...
			const interval = setInterval(() => {
				const elapsed = performance.now() - startedAt;
				const next = Math.min(
					Math.floor(elapsed / PROMPT_CHAR_MS) + 1,
					PROMPT.length,
				);
		...
		const startReply = () => {
			...
			const interval = setInterval(() => {
				const elapsed = performance.now() - startedAt;
				const next = Math.min(
					Math.floor(elapsed / REPLY_WORD_MS) + 1,
					REPLY_WORDS.length,
				);
	...
	const codeTypingActive =
		phase === "code" && typedCodeChars < CODE_TOTAL_CHARS;
	const composerVisible = reducedMotion || !["idle", "code"].includes(phase);
	const editorExited =
		!reducedMotion && ["exit", "chat", "reply", "fade"].includes(phase);
	const chatOpen = !reducedMotion && ["chat", "reply", "fade"].includes(phase);
	const promptText =
		phase === "prompt" || phase === "exit"
			? PROMPT.slice(0, typedPromptChars)
			: "";
