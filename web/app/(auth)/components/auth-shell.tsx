      ...
      <header>
        <nav className='mx-auto flex w-full max-w-[1446px] items-center px-12 py-4 max-sm:px-5 max-lg:px-8'>
          <Link href='/' aria-label='灵犀智学首页' className='flex h-[30px] items-center gap-2'>
            <span className='flex size-6 items-center justify-center rounded-[7px] bg-[var(--text-primary)] text-[11px] font-semibold text-[var(--text-inverse)]'>
              灵
            </span>
            <span className='font-medium text-[13px] text-[var(--text-primary)]'>灵犀智学</span>
          </Link>
        </nav>
      </header>
...
export function AuthLegalFooter({ action }: { action: string }) {
  return (
    <p className='text-center text-[var(--text-muted)] text-xs leading-relaxed'>
      {action}即表示你同意我们的{' '}
      <ChipLink href='/terms' className='inline-flex px-1 text-[var(--text-secondary)] hover:text-[var(--text-primary)]'>
        服务条款
      </ChipLink>{' '}
      和{' '}
      <ChipLink href='/privacy' className='inline-flex px-1 text-[var(--text-secondary)] hover:text-[var(--text-primary)]'>
        隐私政策
      </ChipLink>
    </p>
  )
