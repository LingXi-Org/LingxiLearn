...
/**
 * Local UI-only authentication mode. Development enables it by default; a
 * non-hosted local container may opt in explicitly even when built with
 * NODE_ENV=production. Hosted deployments can never activate this path.
 */
export const isMockAuthEnabled =
  ...
  (isDev ||
    ...
    isTruthy(getEnv('NEXT_PUBLIC_MOCK_AUTH'))) &&
  !isFalsy(getEnv('NEXT_PUBLIC_MOCK_AUTH'))
