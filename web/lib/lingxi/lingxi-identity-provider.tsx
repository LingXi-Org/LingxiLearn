...
export function useLingxiIdentity() {
  ...
  const client = useMemo(
    () => ({
      login: async () =>
        window.location.assign(isMockAuthEnabled ? '/workspace' : identityApi.authUrl('login')),
      register: async () =>
        window.location.assign(isMockAuthEnabled ? '/workspace' : identityApi.authUrl('register')),
      forgotPassword: async () =>
        window.location.assign(
          isMockAuthEnabled ? '/login' : identityApi.authUrl('forgot-password')
        ),
