...
const DEFAULT_SCOPE =
  'openid profile email offline_access roles urn:logto:scope:organizations urn:logto:scope:organization_roles learn.read learn.write'
...
export function lingxiIdentityConfig(): LingxiIdentityConfig {
  return {
    issuer: requiredEnv(
      ...
      process.env.NEXT_PUBLIC_LINGXI_IDENTITY_ISSUER
    ).replace(/\/$/, ''),
    clientId: requiredEnv(
      'NEXT_PUBLIC_LINGXI_IDENTITY_CLIENT_ID',
      process.env.NEXT_PUBLIC_LINGXI_IDENTITY_CLIENT_ID
    ),
    resource: requiredEnv(
      'NEXT_PUBLIC_LINGXI_IDENTITY_RESOURCE',
      process.env.NEXT_PUBLIC_LINGXI_IDENTITY_RESOURCE
    ),
    redirectUri:
      process.env.NEXT_PUBLIC_LINGXI_IDENTITY_REDIRECT_URI ||
...
export class LingxiIdentityClient {
  ...
  private async metadata(): Promise<DiscoveryDocument> {
    this.discovery ??= fetch(`${this.config.issuer}/.well-known/openid-configuration`).then(
      async (response) => {
  ...
  async handleCallback(): Promise<boolean> {
    ...
    if (
      !transaction.state ||
      transaction.state !== query.get('state') ||
      Date.now() - transaction.createdAt > 10 * 60 * 1000
    ) {
      throw new Error('登录状态校验失败，请重新登录。')
    }
    const metadata = await this.metadata()
    await this.exchange(metadata.token_endpoint, {
  ...
  private async exchange(endpoint: string, params: Record<string, string>): Promise<void> {
    const response = await fetch(endpoint, {
      ...
      body: new URLSearchParams({ ...params, resource: this.config.resource }),
    })
    ...
    this.tokens = {
      ...
      idToken: body.id_token || this.tokens?.idToken,
