...
import { getPublicExecutionLog, getPublicExecutionLogScope } from '@/lib/logs/public-queries'
...
type PublicExecutionLog = NonNullable<Awaited<ReturnType<typeof getPublicExecutionLog>>>
...
export interface GetPublicLogResult {
  ...
  log: Omit<PublicExecutionLog, 'workflowState'> & {
...
export const getPublicLog = defineAuthorizedWorkspaceUseCase({
  ...
  resolveContext: async ({ input }: { input: GetPublicLogInput }): Promise<PublicLogContext> => {
    const scope = await getPublicExecutionLogScope(input.runId)
  ...
  execute: async ({ principal, context }): Promise<GetPublicLogResult> => {
    const log = await getPublicExecutionLog(
