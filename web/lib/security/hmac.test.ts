...
describe("hmacSha256Hex", () => {
	it("is deterministic", () => {
		expect(hmacSha256Hex("body", "secret")).toBe(
	...
	it("accepts a Buffer secret and matches the equivalent binary-string secret", () => {
		...
		expect(hmacSha256Hex("Hi There", raw)).toBe(
			hmacSha256Hex("Hi There", raw.toString("binary")),
		);
	});
});
...
describe("hmacSha256Base64", () => {
	it("is deterministic", () => {
		expect(hmacSha256Base64("body", "secret")).toBe(
			hmacSha256Base64("body", "secret"),
		);
	});
