...
describe("isPrivateIp", () => {
	...
	describe("IPv4-compatible IPv6 (::a.b.c.d, RFC 4291 §2.5.5.1, deprecated)", () => {
		...
		it.each([
			...
			"allows IPv4-compatible IPv6 with embedded public IPv4 %s — %s",
			(ip) => {
		...
		it.each([
			[
				"::ffff:1",
				"embedded 255.255.0.1 (Class E reserved) via parts[6]=0xffff",
			],
			...
			"blocks IPv4-compatible IPv6 with Class E embedded IPv4 %s — %s",
			(ip) => {
				expect(isPrivateIp(ip)).toBe(true);
			},
		);
