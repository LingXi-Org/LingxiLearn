...
import { describeRunningCommand } from "@/lib/terminal/protocol";
...
describe("describeRunningCommand", () => {
	...
	it("takes the basename of an absolute program path", () => {
		expect(describeRunningCommand("/opt/homebrew/bin/claude --resume")).toBe(
			"claude",
		);
	});
	...
	it("ignores separators inside quotes", () => {
		expect(
			describeRunningCommand('claude "build && test; deploy | ship"'),
		).toBe("claude");
	...
	it("honors backslash escapes only where the shell does", () => {
		...
		expect(describeRunningCommand('claude "say \\" && rm -rf /"')).toBe(
			"claude",
		);
	...
	it("skips environment assignments and wrapper words", () => {
		...
		expect(describeRunningCommand("sudo /usr/bin/docker compose up")).toBe(
			"docker",
		);
		expect(describeRunningCommand("env FOO=1 nohup python train.py")).toBe(
			"python",
		);
	});

	it("reads the last stage of a pipeline", () => {
		expect(describeRunningCommand("cat log.txt | grep error | less")).toBe(
			"less",
		);
	});
