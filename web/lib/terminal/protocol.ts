...
export type LegacyTerminalToolName =
	(typeof LEGACY_TERMINAL_TOOL_NAMES)[number];
...
const TERMINAL_OPERATION_SET: ReadonlySet<string> = new Set(
	TERMINAL_OPERATIONS,
);

export function isTerminalOperation(
	value: unknown,
): value is TerminalOperation {
...
const TERMINAL_CONTROL_KEY_SET: ReadonlySet<string> = new Set(
	TERMINAL_CONTROL_KEYS,
);

export function isTerminalControlKey(
	value: unknown,
): value is TerminalControlKey {
