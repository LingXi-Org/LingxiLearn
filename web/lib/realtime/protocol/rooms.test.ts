...
describe("roomName", () => {
	it("maps a workflow room to its bare id (backward compatibility)", () => {
		expect(roomName({ type: ROOM_TYPES.WORKFLOW, id: "wf-123" })).toBe(
			"wf-123",
		);
	});
	...
	it("namespaces every non-workflow room type", () => {
		...
		expect(
			roomName({ type: ROOM_TYPES.WORKSPACE_FILE_DOC, id: "file-123" }),
		).toBe("workspace-file-doc:file-123");
	});
	...
	it("keeps the file-doc and file-browser namespaces distinct for the same id", () => {
		...
		expect(parseRoomName(doc)).toEqual({
			type: ROOM_TYPES.WORKSPACE_FILE_DOC,
			id,
		});
		expect(parseRoomName(browser)).toEqual({
	...
	it("never collides a namespaced room with a bare workflow id for real ids", () => {
		...
		const workflow = roomName({
			type: ROOM_TYPES.WORKFLOW,
			id: "a1b2c3d4-uuid",
		});
		const files = roomName({
			type: ROOM_TYPES.WORKSPACE_FILES,
			id: "a1b2c3d4-uuid",
		});
...
describe("parseRoomName", () => {
	...
	it("treats an unprefixed name as a workflow room", () => {
		expect(parseRoomName("bare-uuid")).toEqual({
			type: ROOM_TYPES.WORKFLOW,
			id: "bare-uuid",
		});
	});
	...
	it("does not treat an unknown prefix as a room type", () => {
		expect(parseRoomName("unknown:x")).toEqual({
			type: ROOM_TYPES.WORKFLOW,
			id: "unknown:x",
		});
	});
...
describe("ALL_ROOM_TYPES", () => {
	it("contains every declared room type", () => {
		expect([...ALL_ROOM_TYPES].sort()).toEqual(
			[...Object.values(ROOM_TYPES)].sort(),
		);
	});
});
...
describe("presenceEventName", () => {
	it("keeps the bare historical name for workflow and namespaces the rest", () => {
		...
		expect(presenceEventName(ROOM_TYPES.WORKSPACE_FILES)).toBe(
			"workspace-files:presence-update",
		);
