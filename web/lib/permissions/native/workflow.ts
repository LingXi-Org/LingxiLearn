...
import { db, folder as folderTable, workflow, workspace } from "@/lib/db";
import {
	type PermissionType,
	permissionSatisfies,
...
export async function getWorkflowLockStatus(
	workflowId: string,
): Promise<LockStatus> {
	const [wf] = await db
		.select({
...
export async function isFolderInWorkspace(
	...
	return Boolean(
		folder && folder.workspaceId === workspaceId && !folder.archivedAt,
	);
}
