...
export function isDesktopScopeId(value: unknown): value is string {
	return (
		typeof value === "string" &&
		(DESKTOP_SCOPE_PATTERN.test(value) ||
			PENDING_DESKTOP_SCOPE_PATTERN.test(value))
	);
}
...
export interface SimDesktopBrowserAgentApi {
	 ...
	 */
	clearBrowsingData(
		kinds?: readonly BrowserDataKind[],
	): Promise<BrowserKnownSessionsState>;
	 ...
	 */
	onSessionStatus(
		callback: (alive: boolean, scopeId: string) => void,
	): () => void;
...
const TERMINAL_THEME_OPTIONAL_PALETTE_KEYS = [
	"cursorAccent",
	"selectionForeground",
] as const;
...
function isTerminalThemePalette(value: unknown): value is TerminalThemePalette {
	...
	return (
		TERMINAL_THEME_PALETTE_KEYS.every((key) =>
			isTerminalThemeColor(palette[key]),
		) &&
...
export function isTerminalSelectedProfile(
	value: unknown,
): value is TerminalSelectedProfile {
	if (typeof value !== "object" || value === null) return false;
	...
	return (
		...
		(candidate.lightPalette === undefined ||
			isTerminalThemePalette(candidate.lightPalette)) &&
		(candidate.darkPalette === undefined ||
			isTerminalThemePalette(candidate.darkPalette))
	);
}
...
export function cloneTerminalSelectedProfile(
	...
	return {
		...
		...(profile.lightPalette
			? { lightPalette: { ...profile.lightPalette } }
			: {}),
...
export const DESKTOP_ZOOM_PERCENTS = [
	67, 75, 80, 90, 100, 110, 125, 150, 175, 200,
] as const;

export type DesktopZoomPercent = (typeof DESKTOP_ZOOM_PERCENTS)[number];

export function isDesktopZoomPercent(
	value: unknown,
): value is DesktopZoomPercent {
	return (
		typeof value === "number" &&
		(DESKTOP_ZOOM_PERCENTS as readonly number[]).includes(value)
	);
}
...
export function isTerminalAppearanceTheme(
	value: unknown,
): value is TerminalAppearanceTheme {
...
export type DesktopPreferenceKey = {
	[K in keyof DesktopPreferences]-?: DesktopPreferences[K] extends boolean
		? K
		: never;
