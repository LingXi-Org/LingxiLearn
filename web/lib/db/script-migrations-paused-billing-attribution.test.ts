...
function createStore(candidates: PausedExecutionCandidate[]) {
	...
	const organizationSubscriptions = new Map<
		string,
		SubscriptionCandidate | null
	>([
		[
			"organization-1",
			createSubscription("subscription-1", "organization-1", "team_6000", {
				seats: 4,
			}),
		],
	]);
	...
	const store: PausedBillingAttributionStore = {
		...
		async writeAttribution(update) {
			...
			writes.push({
				id: update.id,
				nextExecutionSnapshot: update.nextExecutionSnapshot,
			});
			rows.set(update.id, {
				...current,
				executionSnapshot: update.nextExecutionSnapshot,
				snapshotBytes: Buffer.byteLength(
					JSON.stringify(update.nextExecutionSnapshot),
					"utf8",
				),
			});
			return true;
...
describe("paused billing attribution serialization", () => {
	it("writes canonical organization attribution with exact inner snapshot bytes", async () => {
		...
		expect((persisted.executionSnapshot as { snapshot: string }).snapshot).toBe(
			expectedSnapshot,
		);
		expect(
			assertFrozenBillingAttributionSnapshot(
				(readInnerSnapshot(persisted).metadata as Record<string, unknown>)
					.billingAttribution,
			),
	...
	it("writes a personal payer with no recognized subscription", async () => {
		...
		harness.personalSubscriptions.set("payer-1", [
			createSubscription("free-1", "payer-1", "free"),
		]);
...
describe("paused billing attribution subscription selection", () => {
	it("uses Enterprise then Team then Pro priority, including suffixed plans", () => {
		const subscriptions = [
			...
			createSubscription("enterprise", "payer-1", "enterprise", {
				status: "past_due",
			}),
		];

		expect(selectFrozenPersonalSubscription(subscriptions)?.id).toBe(
			"enterprise",
		);
		expect(
			selectFrozenPersonalSubscription(subscriptions.slice(0, 2))?.id,
		).toBe("team");
		expect(
			selectFrozenPersonalSubscription(subscriptions.slice(0, 1))?.id,
		).toBe("pro");
	});
...
describe("paused billing attribution safety", () => {
	it("never overwrites valid or malformed existing attribution", async () => {
		...
		expect(harness.rows.get(valid.id)?.executionSnapshot).toBe(
			valid.executionSnapshot,
		);
		expect(harness.rows.get(malformed.id)?.executionSnapshot).toBe(
	...
	it("logs and skips malformed, mismatched, and oversized legacy rows", async () => {
		...
		if (
			typeof mismatched.executionSnapshot !== "object" ||
			mismatched.executionSnapshot === null
		) {
			throw new Error("Expected execution snapshot object");
		}
		const mismatchedInner = readInnerSnapshot(mismatched);
		(mismatchedInner.metadata as Record<string, unknown>).executionId =
			"other-execution";
	...
	it("preserves a concurrent value when full-snapshot compare-and-swap loses", async () => {
		...
		expect(harness.rows.get(candidate.id)?.executionSnapshot).toBe(
			concurrentSnapshot,
		);
	});
	...
	it("is idempotent and processes bounded keyset pages sequentially", async () => {
		const candidates = Array.from({ length: 5 }, (_, index) =>
			createCandidate(`row-${index + 1}`),
		);
		const harness = createStore(candidates);
		let activeLoads = 0;
		let maxActiveLoads = 0;
		const loadWorkspacePayer = harness.store.loadWorkspacePayer.bind(
			harness.store,
		);
	...
	it("throws systemic store failures instead of treating them as malformed rows", async () => {
		...
		harness.store.loadWorkspacePayer = vi
			.fn()
			.mockRejectedValue(new Error("database offline"));

		await expect(
			runPausedBillingAttributionBackfill(harness.store),
