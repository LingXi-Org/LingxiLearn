...
describe("resolveDbUrl", () => {
	...
	it("prefers the role-keyed primary URL over the base", () => {
		...
		expect(resolveDbUrl("DATABASE_URL", "trigger")).toBe(
			"postgres://trigger/db",
		);
	});
	...
	it("resolves the replica variant independently of the primary", () => {
		...
		expect(resolveDbUrl("DATABASE_REPLICA_URL", "trigger")).toBe(
			"postgres://trigger-replica/db",
		);
		expect(resolveDbUrl("DATABASE_REPLICA_URL", "web")).toBe(
			"postgres://replica/db",
		);
	});
