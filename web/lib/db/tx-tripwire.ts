...
function resolveMode(): TripwireMode {
	...
	if (override === "off" || override === "warn" || override === "throw")
		return override;
...
function report(poolName: string, query: string): void {
	...
	if (lastLogged !== undefined && now - lastLogged < WARN_DEDUPE_WINDOW_MS)
		return;
...
export function instrumentPoolClient<T extends InstrumentablePoolClient>(
	...
	target.unsafe = (...args: unknown[]) => {
		if (transactionContext.getStore()) {
			report(
				poolName,
				typeof args[0] === "string" ? args[0] : "(non-string query)",
			);
		}
	...
	target.begin = (...args: unknown[]) => {
		if (transactionContext.getStore()) {
			report(
				poolName,
				"BEGIN (nested transaction opened on the global client)",
			);
		}
