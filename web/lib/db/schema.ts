...
export const folder = pgTable(
	...
	(table) => ({
		...
		workspaceResourceParentIdx: index(
			"folder_workspace_resource_parent_idx",
		).on(table.workspaceId, table.resourceType, table.parentId),
		parentSortIdx: index("folder_parent_sort_idx").on(
			table.parentId,
			table.sortOrder,
		),
...
export const pinnedItem = pgTable(
	...
	(table) => ({
		userWorkspaceIdx: index("pinned_item_user_workspace_idx").on(
			table.userId,
			table.workspaceId,
		),
		resourceIdx: index("pinned_item_resource_idx").on(
			table.resourceType,
			table.resourceId,
		),
...
export const workflowExecutionLogs = pgTable(
	...
	(table) => ({
		...
		deploymentVersionIdIdx: index(
			"workflow_execution_logs_deployment_version_id_idx",
		).on(table.deploymentVersionId),
		...
		workspaceStartedAtIdDescIdx: index(
			...
			table.workspaceId,
			sql`${table.startedAt} DESC NULLS LAST`,
			sql`${table.id} DESC`,
		),
		workspaceCostTotalIdx: index(
			"workflow_execution_logs_workspace_cost_total_idx",
		).on(table.workspaceId, table.costTotal),
		modelsUsedIdx: index("workflow_execution_logs_models_used_idx").using(
			"gin",
			table.modelsUsed,
		),
		workspaceEndedAtIdIdx: index(
		...
		runningExecutionDeadlineIdx: index(
			"workflow_execution_logs_running_deadline_idx",
		)
			.on(table.executionDeadlineAt)
			.where(
				sql`${table.status} = 'running' AND ${table.executionDeadlineAt} IS NOT NULL`,
			),
...
export const executionLargeValues = pgTable(
	...
	(table) => ({
		ownerExecutionIdIdx: index(
			"execution_large_values_owner_execution_id_idx",
		).on(table.ownerExecutionId),
		 ...
		 */
		workflowIdIdx: index("execution_large_values_workflow_id_idx").on(
			table.workflowId,
		),
	}),
);
...
export const executionLargeValueReferences = pgTable(
	...
	(table) => ({
		...
		workflowIdIdx: index("execution_large_value_references_workflow_id_idx").on(
			table.workflowId,
		),
	}),
);
...
export const executionLargeValueDependencies = pgTable(
	...
	(table) => ({
		...
		workspaceParentKeyIdx: index(
			"execution_large_value_dependencies_workspace_parent_key_idx",
		).on(table.workspaceId, table.parentKey),
		workspaceChildKeyIdx: index(
			"execution_large_value_dependencies_workspace_child_key_idx",
		).on(table.workspaceId, table.childKey),
	}),
);
...
export const pausedExecutions = pgTable(
	"paused_executions",
	{
		...
		automaticResumeRetryCount: integer("automatic_resume_retry_count")
			.notNull()
			.default(0),
	...
	(table) => ({
		workflowIdx: index("paused_executions_workflow_id_idx").on(
			table.workflowId,
		),
		statusIdx: index("paused_executions_status_idx").on(table.status),
		executionUnique: uniqueIndex("paused_executions_execution_id_unique").on(
			table.executionId,
		),
...
export const resumeQueue = pgTable(
	...
	(table) => ({
		...
		newExecutionIdx: index("resume_queue_new_execution_idx").on(
			table.newExecutionId,
		),
	}),
);
...
export const workspaceEnvironment = pgTable(
	...
	(table) => ({
		workspaceUnique: uniqueIndex("workspace_environment_workspace_unique").on(
			table.workspaceId,
		),
	}),
);
...
export const settings = pgTable("settings", {
	...
	// UI preferences
	showTrainingControls: boolean("show_training_controls")
		.notNull()
		.default(false),
	superUserModeEnabled: boolean("super_user_mode_enabled")
		.notNull()
		.default(true),
	mothershipEnvironment: text("mothership_environment")
		.notNull()
		.default("default"),
	...
	copilotAutoAllowedTools: jsonb("copilot_auto_allowed_tools")
		.notNull()
		.default("[]"),
...
export const workflowSchedule = pgTable(
	"workflow_schedule",
	{
		...
		sourceWorkspaceId: text("source_workspace_id").references(
			() => workspace.id,
			{
				onDelete: "cascade",
			},
		...
		mountedSecrets: jsonb("mounted_secrets")
			.$type<string[]>()
			.notNull()
			.default([]),
		jobHistory:
			jsonb("job_history").$type<
	...
	(table) => {
		return {
			workflowBlockUnique: uniqueIndex(
				...
				.where(sql`${table.archivedAt} IS NULL`),
			workflowDeploymentIdx: index(
				"workflow_schedule_workflow_deployment_idx",
			).on(table.workflowId, table.deploymentVersionId),
...
export const jobExecutionLogs = pgTable(
	...
	(table) => ({
		scheduleIdIdx: index("job_execution_logs_schedule_id_idx").on(
			table.scheduleId,
		),
		workspaceStartedAtIdx: index(
		...
		executionIdUnique: uniqueIndex("job_execution_logs_execution_id_unique").on(
			table.executionId,
		),
	...
	(table) => {
		return {
			...
			routingKeyActiveIdx: index("webhook_routing_key_active_idx")
				...
				.where(
					sql`${table.archivedAt} IS NULL AND ${table.routingKey} IS NOT NULL`,
				),
			...
			workflowBlockUpdatedDescIdx: index(
				"idx_webhook_on_workflow_id_block_id_updated_at_desc",
			).on(table.workflowId, table.blockId, table.updatedAt.desc()),
			...
			candidateRegistrationUnique: uniqueIndex(
				"webhook_candidate_registration_unique",
			)
				.on(table.workflowId, table.blockId)
				.where(
					sql`${table.registrationStatus} = 'candidate' AND ${table.blockId} IS NOT NULL`,
				),
			registrationGenerationIdx: index(
...
export const simTriggerState = pgTable(
	...
	(table) => ({
		pk: primaryKey({
			columns: [table.workflowId, table.blockId, table.scopeKey],
		}),
	}),
);
...
export const apiKey = pgTable(
	...
	(table) => ({
		...
		workspaceTypeIdx: index("api_key_workspace_type_idx").on(
			table.workspaceId,
			table.type,
		),
...
export const billingEntityTypeEnum = pgEnum("billing_entity_type", [
	"user",
	"organization",
]);
...
export const userStats = pgTable("user_stats", {
	...
	totalManualExecutions: integer("total_manual_executions")
		.notNull()
		.default(0),
	/** @deprecated Retired usage counter; derive from usage_log. */
	...
	/** @deprecated Retired usage counter; derive from usage_log. */
	totalScheduledExecutions: integer("total_scheduled_executions")
		.notNull()
		.default(0),
	/** @deprecated Retired usage counter; derive from usage_log. */
	...
	currentPeriodCopilotCost: decimal("current_period_copilot_cost")
		.notNull()
		.default("0"),
	...
	totalCopilotTokens: bigint("total_copilot_tokens", { mode: "number" })
		.notNull()
		.default(0),
	...
	currentPeriodMcpCopilotCost: decimal("current_period_mcp_copilot_cost")
		.notNull()
		.default("0"),
	/**
...
export const customTools = pgTable(
	...
	(table) => ({
		workspaceIdIdx: index("custom_tools_workspace_id_idx").on(
			table.workspaceId,
		),
...
export const skillMember = pgTable(
	...
	(table) => ({
		...
		uniqueMembership: uniqueIndex("skill_member_unique").on(
			table.skillId,
			table.userId,
		),
	}),
...
export const mothershipSettings = pgTable(
	"mothership_settings",
	{
		...
		customToolRefs: jsonb("custom_tool_refs")
			.notNull()
			.default(sql`'[]'::jsonb`),
	...
	(table) => ({
		workspaceIdIdx: index("mothership_settings_workspace_id_idx").on(
			table.workspaceId,
		),
	}),
);
...
export const organization = pgTable("organization", {
	 ...
	 */
	securityPolicyVersion: integer("security_policy_version")
		.notNull()
		.default(1),
...
export const organizationMemberUsageLimit = pgTable(
	...
	(table) => ({
		...
		organizationIdIdx: index("org_member_usage_limit_organization_id_idx").on(
			table.organizationId,
		),
	}),
);
...
export const invitationMembershipIntentEnum = pgEnum(
	"invitation_membership_intent",
	["internal", "external"],
);

export type InvitationMembershipIntent =
	(typeof invitationMembershipIntentEnum.enumValues)[number];
...
export const invitation = pgTable(
	...
	(table) => ({
		...
		pendingPerOrgEmailUnique: uniqueIndex("invitation_pending_email_org_unique")
			...
			.where(
				sql`${table.status} = 'pending' AND ${table.organizationId} IS NOT NULL`,
			),
	}),
);
...
export const workspace = pgTable(
	"workspace",
	{
		...
		inboxMountedSecrets: jsonb("inbox_mounted_secrets")
			.$type<string[]>()
			.notNull()
			.default([]),
		archivedAt: timestamp("archived_at"),
	...
	(table) => ({
		...
		organizationIdIdx: index("workspace_organization_id_idx").on(
			table.organizationId,
		),
...
export const workspaceForkResourceTypeEnum = pgEnum(
	"workspace_fork_resource_type",
	[
		"workflow",
		"oauth_credential",
...
export const workspaceForkResourceMap = pgTable(
	...
	(table) => ({
		childWorkspaceIdx: index("workspace_fork_resource_map_child_ws_idx").on(
			table.childWorkspaceId,
		),
		childWorkspaceTypeIdx: index(
			"workspace_fork_resource_map_child_ws_type_idx",
		).on(table.childWorkspaceId, table.resourceType),
		childTypeParentUnique: uniqueIndex(
			"workspace_fork_resource_map_child_type_parent_unique",
		).on(table.childWorkspaceId, table.resourceType, table.parentResourceId),
	}),
);
...
export const workspaceForkBlockMap = pgTable(
	...
	(table) => ({
		...
		childWsParentWorkflowIdx: index(
			"workspace_fork_block_map_child_ws_parent_wf_idx",
		).on(table.childWorkspaceId, table.parentWorkflowId),
		childWsChildWorkflowIdx: index(
			"workspace_fork_block_map_child_ws_child_wf_idx",
		).on(table.childWorkspaceId, table.childWorkflowId),
	}),
);
...
export const workspaceForkPromoteDirectionEnum = pgEnum(
	"workspace_fork_promote_direction",
	["push", "pull"],
);
...
export const workspaceForkPromoteRun = pgTable(
	...
	(table) => ({
		...
		childWorkspaceTargetUnique: uniqueIndex(
			"workspace_fork_promote_run_child_ws_target_unique",
		).on(table.childWorkspaceId, table.targetWorkspaceId),
...
export const backgroundWorkStatusValueEnum = pgEnum(
	"background_work_status_value",
...
export const uploadSessionMethodEnum = pgEnum("upload_session_method", [
	"put",
	"multipart",
]);
...
export const uploadSession = pgTable(
	...
	(table) => ({
		tokenHashUnique: uniqueIndex("upload_session_token_hash_unique").on(
			table.tokenHash,
		),
		finalKeyUnique: uniqueIndex("upload_session_final_key_unique").on(
			table.finalKey,
		),
...
export interface WorkspaceFileSecretProvenanceEntry
	extends DurableSecretProvenanceEntry {
	sourceUserId: string;
}
...
export interface TableRowSecretProvenanceEntry
	extends DurableSecretProvenanceEntry {
	columnId: string;
}
...
export const publicShare = pgTable(
	...
	(table) => ({
		...
		workspaceIdIdx: index("public_share_workspace_id_idx").on(
			table.workspaceId,
		),
	}),
);
...
export const invitationWorkspaceGrant = pgTable(
	...
	(table) => ({
		invitationWorkspaceUnique: uniqueIndex(
			"invitation_workspace_grant_unique",
		).on(table.invitationId, table.workspaceId),
		workspaceIdIdx: index("invitation_workspace_grant_workspace_id_idx").on(
			table.workspaceId,
		),
	}),
);
...
export const permissions = pgTable(
	...
	(table) => ({
		...
		entityIdx: index("permissions_entity_idx").on(
			table.entityType,
			table.entityId,
		),

		// User + entity type queries - get user's permissions for all workspaces
		userEntityTypeIdx: index("permissions_user_entity_type_idx").on(
			table.userId,
			table.entityType,
		),
...
export const knowledgeBase = pgTable(
	...
	(table) => ({
		...
		userWorkspaceIdx: index("kb_user_workspace_idx").on(
			table.userId,
			table.workspaceId,
		),
...
export const copilotChats = pgTable(
	...
	(table) => ({
		...
		userWorkflowIdx: index("copilot_chats_user_workflow_idx").on(
			table.userId,
			table.workflowId,
		),
		...
		userWorkspaceDeletedPartialIdx: index(
...
export const copilotWorkflowReadHashes = pgTable(
	...
	(table) => ({
		chatIdIdx: index("copilot_workflow_read_hashes_chat_id_idx").on(
			table.chatId,
		),
		workflowIdIdx: index("copilot_workflow_read_hashes_workflow_id_idx").on(
			table.workflowId,
		),
		chatWorkflowUnique: uniqueIndex(
			"copilot_workflow_read_hashes_chat_workflow_unique",
		).on(table.chatId, table.workflowId),
	}),
);
...
export const copilotRuns = pgTable(
	...
	(table) => ({
		executionIdIdx: index("copilot_runs_execution_id_idx").on(
			table.executionId,
		),
		parentRunIdIdx: index("copilot_runs_parent_run_id_idx").on(
			table.parentRunId,
		),
		...
		workspaceIdIdx: index("copilot_runs_workspace_id_idx").on(
			table.workspaceId,
		),
		statusIdx: index("copilot_runs_status_idx").on(table.status),
		chatExecutionIdx: index("copilot_runs_chat_execution_idx").on(
			table.chatId,
			table.executionId,
		),
		...
		workspaceCompletedAtIdIdx: index(
		...
		streamIdUnique: uniqueIndex("copilot_runs_stream_id_unique").on(
			table.streamId,
		),
	}),
);
...
export const copilotRunCheckpoints = pgTable(
	...
	(table) => ({
		...
		pendingToolCallIdIdx: index(
			"copilot_run_checkpoints_pending_tool_call_id_idx",
		).on(table.pendingToolCallId),
		runPendingUnique: uniqueIndex(
			"copilot_run_checkpoints_run_pending_tool_unique",
		).on(table.runId, table.pendingToolCallId),
	}),
);
...
export const copilotAsyncToolCalls = pgTable(
	"copilot_async_tool_calls",
	{
		...
		permissionDecision: copilotToolPermissionDecisionEnum(
			"permission_decision",
		),
	...
	(table) => ({
		...
		checkpointIdIdx: index("copilot_async_tool_calls_checkpoint_id_idx").on(
			table.checkpointId,
		),
		toolCallIdIdx: index("copilot_async_tool_calls_tool_call_id_idx").on(
			table.toolCallId,
		),
		statusIdx: index("copilot_async_tool_calls_status_idx").on(table.status),
		runStatusIdx: index("copilot_async_tool_calls_run_status_idx").on(
			table.runId,
			table.status,
		),
		toolCallUnique: uniqueIndex(
...
export const copilotFeedback = pgTable(
	...
	(table) => ({
		...
		userChatIdx: index("copilot_feedback_user_chat_idx").on(
			table.userId,
			table.chatId,
		),

		// Query patterns
		isPositiveIdx: index("copilot_feedback_is_positive_idx").on(
			table.isPositive,
		),
...
export const mcpServerOauth = pgTable(
	...
	(table) => ({
		serverUnique: uniqueIndex("mcp_server_oauth_server_unique").on(
			table.mcpServerId,
		),
...
export const ssoProvider = pgTable(
	...
	(table) => ({
		...
		providerIdUnique: uniqueIndex("sso_provider_provider_id_unique").on(
			table.providerId,
		),
		domainIdx: index("sso_provider_domain_idx").on(table.domain),
		userIdIdx: index("sso_provider_user_id_idx").on(table.userId),
		organizationIdIdx: index("sso_provider_organization_id_idx").on(
			table.organizationId,
		),
	}),
);
...
export const ssoDomain = pgTable(
	...
	(table) => ({
		organizationIdIdx: index("sso_domain_organization_id_idx").on(
			table.organizationId,
		),
...
export const workflowMcpServer = pgTable(
	...
	(table) => ({
		workspaceIdIdx: index("workflow_mcp_server_workspace_id_idx").on(
			table.workspaceId,
		),
		createdByIdx: index("workflow_mcp_server_created_by_idx").on(
			table.createdBy,
		),
		deletedAtIdx: index("workflow_mcp_server_deleted_at_idx").on(
			table.deletedAt,
		),
		workspaceDeletedAtPartialIdx: index(
...
export const workflowMcpTool = pgTable(
	...
	(table) => ({
		...
		workflowIdIdx: index("workflow_mcp_tool_workflow_id_idx").on(
			table.workflowId,
		),
		serverWorkflowUnique: uniqueIndex(
...
export const customBlock = pgTable(
	"custom_block",
	{
		...
		inputs:
			json("inputs").$type<
				Array<{ id: string; placeholder?: string; required?: boolean }>
			>(),
		/**
		...
		outputs:
			json("outputs").$type<
				Array<{ blockId: string; path: string; name: string }>
			>(),
		enabled: boolean("enabled").notNull().default(true),
		createdBy: text("created_by").references(() => user.id, {
			onDelete: "set null",
	...
	(table) => ({
		organizationIdIdx: index("custom_block_organization_id_idx").on(
			table.organizationId,
		),
...
export const usageLog = pgTable(
	...
	(table) => ({
		userCreatedAtIdx: index("usage_log_user_created_at_idx").on(
			table.userId,
			table.createdAt,
		),
...
export const credential = pgTable(
	...
	(table) => ({
		...
		envOwnerUserIdIdx: index("credential_env_owner_user_id_idx").on(
			table.envOwnerUserId,
		),
		...
		workspacePersonalEnvUnique: uniqueIndex(
...
export const credentialMemberRoleEnum = pgEnum("credential_member_role", [
	"admin",
	"member",
]);
...
export const credentialMember = pgTable(
	...
	(table) => ({
		...
		uniqueMembership: uniqueIndex("credential_member_unique").on(
			table.credentialId,
			table.userId,
		),
	}),
...
export const permissionGroup = pgTable(
	...
	(table) => ({
		...
		defaultGroupUnique: uniqueIndex(
...
export const permissionGroupWorkspace = pgTable(
	...
	(table) => ({
		workspaceIdIdx: index("permission_group_workspace_workspace_id_idx").on(
			table.workspaceId,
		),
		groupWorkspaceUnique: uniqueIndex(
			"permission_group_workspace_group_workspace_unique",
		).on(table.permissionGroupId, table.workspaceId),
	}),
);
...
export const permissionGroupMember = pgTable(
	...
	(table) => ({
		permissionGroupIdIdx: index("permission_group_member_group_id_idx").on(
			table.permissionGroupId,
		),
		groupUserUnique: uniqueIndex(
			"permission_group_member_group_user_unique",
		).on(table.permissionGroupId, table.userId),
...
export const asyncJobs = pgTable(
	...
	(table) => ({
		statusStartedAtIdx: index("async_jobs_status_started_at_idx").on(
			table.status,
			table.startedAt,
		),
		...
		schedulePendingRunAtIdx: index("async_jobs_schedule_pending_run_at_idx")
			...
			.where(
				sql`${table.type} = 'schedule-execution' AND ${table.status} = 'pending'`,
			),
		scheduleProcessingStartedAtIdx: index(
			"async_jobs_schedule_processing_started_at_idx",
		)
			.on(table.startedAt, table.id)
			.where(
				sql`${table.type} = 'schedule-execution' AND ${table.status} = 'processing'`,
			),
	}),
);
...
export const knowledgeConnector = pgTable(
	...
	(table) => ({
		knowledgeBaseIdIdx: index("kc_knowledge_base_id_idx").on(
			table.knowledgeBaseId,
		),
		statusNextSyncIdx: index("kc_status_next_sync_idx").on(
			table.status,
			table.nextSyncAt,
		),
...
export const userTableDefinitions = pgTable(
	"user_table_definitions",
	{
		 ...
		 */
		rowsVersion: bigint("rows_version", { mode: "number" })
			.notNull()
			.default(0),
		/**
		 * @remarks
	...
	(table) => ({
		workspaceIdIdx: index("user_table_def_workspace_id_idx").on(
			table.workspaceId,
		),
...
export const userTableRows = pgTable(
	...
	(table) => ({
		 ...
		 * O(all rows) per page.
		 */
		tableIdIdIdx: index("user_table_rows_table_id_id_idx").on(
			table.tableId,
			table.id,
		),
	}),
...
export const tableViews = pgTable(
	...
	(table) => ({
		...
		tableCreatedIdx: index("table_views_table_created_idx").on(
			table.tableId,
			table.createdAt,
		),
...
export const tableJobs = pgTable(
	...
	(table) => ({
		...
		watchdogIdx: index("table_jobs_watchdog_idx").on(
			table.status,
			table.updatedAt,
		),
		tableStartedIdx: index("table_jobs_table_started_idx").on(
			table.tableId,
			table.startedAt,
		),
	}),
...
export const tableRowExecutions = pgTable(
	"table_row_executions",
	{
		...
		error: text("error"),
		runningBlockIds: text("running_block_ids")
			.array()
			.notNull()
			.default(sql`'{}'::text[]`),
...
export const tableRunDispatches = pgTable(
	...
	(table) => ({
		activeIdx: index("table_run_dispatches_active_idx").on(
			table.tableId,
			table.status,
		),
		watchdogIdx: index("table_run_dispatches_watchdog_idx").on(
			table.status,
			table.requestedAt,
		),
	}),
...
export const mothershipInboxAllowedSender = pgTable(
	...
	(table) => ({
		wsEmailIdx: uniqueIndex("inbox_sender_ws_email_idx").on(
			table.workspaceId,
			table.email,
		),
	}),
...
export const mothershipInboxTask = pgTable(
	...
	(table) => ({
		wsCreatedAtIdx: index("inbox_task_ws_created_at_idx").on(
			table.workspaceId,
			table.createdAt,
		),
		wsStatusIdx: index("inbox_task_ws_status_idx").on(
			table.workspaceId,
			table.status,
		),
		responseMsgIdIdx: index("inbox_task_response_msg_id_idx").on(
			table.responseMessageId,
		),
		emailMsgIdIdx: index("inbox_task_email_msg_id_idx").on(
 ...
 */
export const academyCertStatusEnum = pgEnum("academy_cert_status", [
	"active",
	"revoked",
	"expired",
]);
...
export const academyCertificate = pgTable(
	...
	(table) => ({
		...
		certNumberIdx: index("academy_certificate_number_idx").on(
...
export type DataDrainDestination =
	(typeof dataDrainDestinationEnum.enumValues)[number];

export const dataDrainCadenceEnum = pgEnum("data_drain_cadence", [
	"hourly",
	"daily",
]);
...
export type DataDrainRunStatus =
	(typeof dataDrainRunStatusEnum.enumValues)[number];

export const dataDrainRunTriggerEnum = pgEnum("data_drain_run_trigger", [
	"cron",
	"manual",
]);

export type DataDrainRunTrigger =
	(typeof dataDrainRunTriggerEnum.enumValues)[number];
...
export const dataDrains = pgTable(
	...
	(table) => ({
		...
		orgNameUnique: uniqueIndex("data_drains_org_name_unique").on(
			table.organizationId,
			table.name,
		),
	}),
...
export const dataDrainRuns = pgTable(
	...
	(table) => ({
		drainStartedIdx: index("data_drain_runs_drain_started_idx").on(
			table.drainId,
			table.startedAt,
		),
	}),
...
export const sandboxLanguageEnum = pgEnum("sandbox_language", [
	"javascript",
	"python",
]);

export type SandboxLanguageValue =
	(typeof sandboxLanguageEnum.enumValues)[number];
...
export type SandboxImageStatusValue =
	(typeof sandboxImageStatusEnum.enumValues)[number];
...
export const workspaceSandbox = pgTable(
	"workspace_sandbox",
	{
		...
		dependencies: jsonb("dependencies")
			...
			.notNull()
			.default(sql`'[]'::jsonb`),
		cliTools: jsonb("cli_tools")
			.$type<string[]>()
			.notNull()
			.default(sql`'[]'::jsonb`),
		systemPackages: jsonb("system_packages")
			.$type<string[]>()
			.notNull()
			.default(sql`'[]'::jsonb`),
	...
	(table) => ({
		...
		workspaceIdx: index("workspace_sandbox_workspace_idx").on(
			table.workspaceId,
		),
...
export const sandboxImage = pgTable(
	"sandbox_image",
	{
		...
		materializationGeneration: bigint("materialization_generation", {
			mode: "number",
		}),
