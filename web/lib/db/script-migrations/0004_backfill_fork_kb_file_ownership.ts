...
export function assertForkKnowledgeBaseOwnershipPostDrainAck(
...
export async function reconcileForkKnowledgeBaseFileOwnership(
	...
	if (!Number.isInteger(batchSize) || batchSize <= 0) {
		throw new Error(
			"Fork KB ownership repair batch size must be a positive integer",
		);
	}
	...
	for (;;) {
		const documentIds = await store.listCandidateDocumentIds(
			afterId,
			batchSize,
		);
		...
		if (documentIds.length > batchSize) {
			throw new Error(
				"Fork KB ownership repair store returned an oversized page",
			);
		}
		...
		if (!lastId || lastId <= afterId) {
			throw new Error(
				"Fork KB ownership repair store returned a non-advancing page",
			);
		}
