...
type ParsedPausedSnapshot =
	| ParsedMissingAttribution
	| ParsedExistingAttribution;
...
function parseSnapshotDate(value: unknown, field: string): string {
	if (!isNonEmptyString(value)) {
		return legacyError(
			`Billing attribution ${field} must be a non-empty ISO date string`,
		);
	}
	const date = new Date(value);
	if (!Number.isFinite(date.getTime())) {
		return legacyError(
			`Billing attribution ${field} is not a valid ISO date string`,
		);
	}
 ...
 */
export function assertFrozenBillingAttributionSnapshot(
	value: unknown,
	...
	if (
		...
		return legacyError(
			"Billing attribution organization must be a non-empty string or null",
		);
	}
	...
	if (!isRecordLike(value.billingEntity)) {
		return legacyError(
			"Billing attribution snapshot is missing its billing entity",
		);
	}
	...
	if (
		...
		!isNonEmptyString(entityId)
	) {
		return legacyError(
			"Billing attribution snapshot has an invalid billing entity",
		);
	}
	if (
		(entityType === "organization" &&
			(organizationId === null || entityId !== organizationId)) ||
		(entityType === "user" &&
			(organizationId !== null || entityId !== value.billedAccountUserId))
	) {
	...
	if (!isRecordLike(value.billingPeriod)) {
		return legacyError(
			"Billing attribution snapshot is missing its billing period",
		);
	}
	const periodStart = parseSnapshotDate(
		value.billingPeriod.start,
		"billingPeriod.start",
	);
	const periodEnd = parseSnapshotDate(
		value.billingPeriod.end,
		"billingPeriod.end",
	);
	if (new Date(periodEnd) <= new Date(periodStart)) {
		return legacyError(
			"Billing attribution billing period must end after it starts",
		);
	}
	...
	if (value.payerSubscription !== null) {
		if (!isRecordLike(value.payerSubscription)) {
			return legacyError(
				"Billing attribution payer subscription must be an object or null",
			);
		}
		...
		if (
			...
			return legacyError(
				"Billing attribution subscription period must end after it starts",
			);
		}
		if (subscription.referenceId !== entityId) {
			return legacyError(
				"Billing attribution subscription does not belong to its billing entity",
			);
		}
...
function serializeSubscription(
	...
	if (
		...
		return legacyError(
			`Subscription ${subscription.id || "<missing>"} is invalid`,
		);
	}
	...
	const periodStart = parseDatabaseTimestamp(
		subscription.periodStart,
		"period_start",
	);
	const periodEnd = parseDatabaseTimestamp(
		subscription.periodEnd,
		"period_end",
	);
	if (
		...
		return legacyError(
			`Subscription ${subscription.id} has an invalid billing period`,
		);
	}
...
function parsePausedSnapshot(
	candidate: PausedExecutionCandidate,
	...
	if (
		Buffer.byteLength(executionSnapshot.snapshot, "utf8") >
		MAX_PAUSED_BILLING_SNAPSHOT_BYTES
	) {
		return legacyError(
			"Paused execution inner snapshot exceeds the migration byte limit",
		);
	}
	...
	if (
		...
		return legacyError(
			"Paused execution snapshot is missing actor or execution bindings",
		);
	}
	if (
		...
		return legacyError(
			"Paused execution snapshot does not match its durable row",
		);
	}

	if (Object.hasOwn(metadata, "billingAttribution")) {
		const attribution = assertFrozenBillingAttributionSnapshot(
			metadata.billingAttribution,
		);
		if (
...
async function resolveFrozenAttribution(
	...
	if (!payer || !isNonEmptyString(payer.billedAccountUserId)) {
		return legacyError(
			`Unable to resolve billing payer for workspace ${parsed.workspaceId}`,
		);
	}
	if (
		payer.organizationId !== null &&
		!isNonEmptyString(payer.organizationId)
	) {
		return legacyError(
			`Workspace ${parsed.workspaceId} has an invalid organization payer`,
		);
	}
	...
	const payerSubscription = serializeSubscription(
		subscription,
		expectedReferenceId,
	);
	const billingPeriod =
		payerSubscription?.periodStart && payerSubscription.periodEnd
			? {
					start: payerSubscription.periodStart,
					end: payerSubscription.periodEnd,
				}
...
function buildAttributedExecutionSnapshot(
	...
	if (
		...
		return legacyError(
			"Resolved attribution does not match the persisted paused snapshot",
		);
	}
	...
	if (
		Buffer.byteLength(serializedSnapshot, "utf8") >
		MAX_PAUSED_BILLING_SNAPSHOT_BYTES
	) {
		return legacyError(
			"Attributed paused execution snapshot exceeds the migration byte limit",
		);
	}
	...
	if (
		Buffer.byteLength(JSON.stringify(executionSnapshot), "utf8") >
		MAX_PAUSED_BILLING_SNAPSHOT_BYTES
	) {
		return legacyError(
			"Attributed paused execution envelope exceeds the migration byte limit",
		);
	}
...
function normalizeCount(value: number | string): number {
	...
	if (!Number.isSafeInteger(normalized) || normalized < 0) {
		throw new Error(
			`Invalid paused execution snapshot byte count: ${String(value)}`,
		);
	}
	return normalized;
}
 ...
 * postgres client.
 */
export function createPausedBillingAttributionStore(
	sql: Sql,
): PausedBillingAttributionStore {
	return {
      ...
      `;
			if (rows.length > 1) {
				throw new Error(
					`Conditional paused execution update affected ${rows.length} rows`,
				);
			}
...
function assertKeysetPage(
	...
	if (rows.length > batchSize) {
		throw new Error(
			`Paused execution store exceeded the ${batchSize}-row keyset bound`,
		);
	}
	...
	for (const row of rows) {
		if (
			...
			(previousId !== undefined && row.id <= previousId)
		) {
			throw new Error(
				"Paused execution store returned a non-increasing keyset page",
			);
		}
		previousId = row.id;
	}
...
export async function runPausedBillingAttributionBackfill(
	...
	if (!Number.isSafeInteger(batchSize) || batchSize < 1) {
		throw new Error(
			"Paused billing attribution batch size must be a positive integer",
		);
	}
	...
	for (;;) {
		...
		for (const row of rows) {
			...
			try {
				...
				const nextExecutionSnapshot = buildAttributedExecutionSnapshot(
					parsed,
					attribution,
				);
				...
				if (!(error instanceof LegacyPausedBillingAttributionError))
					throw error;
				summary.malformed += 1;
				logger.warn(
					"Skipping malformed legacy paused execution during attribution backfill",
					{
...
export const backfillPausedBillingAttribution: ScriptMigration = {
	...
	async up(sql) {
		await runPausedBillingAttributionBackfill(
			createPausedBillingAttributionStore(sql),
		);
	},
};
