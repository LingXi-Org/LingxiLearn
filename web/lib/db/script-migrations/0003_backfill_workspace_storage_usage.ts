...
export async function reconcileWorkspaceStorageAccounting(
	...
	const {
		batchSize = WORKSPACE_STORAGE_RECONCILIATION_BATCH_SIZE,
		reconcilePayers = true,
	} = options;
	if (!Number.isInteger(batchSize) || batchSize <= 0) {
		throw new Error(
			"Workspace storage reconciliation batch size must be a positive integer",
		);
	}
...
export function createPostgresStorageReconciliationStore(
	sql: Sql,
): StorageReconciliationStore {
	return {
      ...
      `;
        ...
        `;
				if (Number(invalid?.invalid_count ?? 0) > 0) {
					throw new Error(
						"Cannot reconcile workspace storage: negative source metadata size",
					);
				}
      ...
      `;
        ...
        `;
				...
				const [total] = await tx<
					Array<{ storage_used_bytes: number | string }>
				>`
...
export const backfillWorkspaceStorageUsage: ScriptMigration = {
	...
	async up(sql) {
		 ...
		 */
		await reconcileWorkspaceStorageAccounting(
