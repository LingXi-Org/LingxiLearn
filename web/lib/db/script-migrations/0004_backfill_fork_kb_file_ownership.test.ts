	...
	const query = vi.fn((strings: TemplateStringsArray, ...values: unknown[]) => {
		...
		if (text.includes("INSERT INTO workspace_files")) {
			return Promise.resolve(
				(options.insertedKeys ?? []).map((key) => ({ key })),
			);
		}
...
describe("fork knowledge-base ownership reconciliation", () => {
	it("processes bounded keyset pages sequentially and aggregates unresolved rows", async () => {
		...
		await expect(
			reconcileForkKnowledgeBaseFileOwnership(store, { batchSize: 2 }),
		).resolves.toEqual({ scanned: 3, inserted: 2, unresolved: 1 });
	...
	it("rejects oversized and non-advancing store pages", async () => {
		...
		await expect(
			reconcileForkKnowledgeBaseFileOwnership(nonAdvancing),
	...
	it("is idempotent after the first run records every uncontested binding", async () => {
		...
		await expect(
			reconcileForkKnowledgeBaseFileOwnership(store),
		).resolves.toEqual({
			scanned: 2,
			inserted: 2,
			unresolved: 0,
	...
	it("selects only active documents with canonical deterministic fork identities", async () => {
		...
		const store = createPostgresForkKnowledgeBaseOwnershipRepairStore(
			harness.sql,
		);

		await expect(
			store.listCandidateDocumentIds("fork_document_0", 250),
		).resolves.toEqual(["fork_document_a"]);
	...
	it("inserts only uncontested bindings and reports every still-unbound candidate", async () => {
		...
		await expect(
			store.repairDocumentIds(["fork_document_a", "fork_document_b"]),
		).resolves.toEqual({
			inserted: 1,
			unresolved: 1,
		});
		...
		expect(insert).toContain(
			"other_kb.workspace_id IS DISTINCT FROM kb.workspace_id",
		);
		...
		expect(harness.queries[0].values).toEqual([
			["fork_document_a", "fork_document_b"],
		]);
		...
		expect(harness.queries[1].values).toEqual([
			["fork_document_a", "fork_document_b"],
		]);
	});
	...
	it("requires the post-drain acknowledgement and fails unresolved operator runs", () => {
		expect(() =>
			assertForkKnowledgeBaseOwnershipPostDrainAck(undefined),
		).toThrow("only after old app instances are drained");
		expect(() =>
			assertForkKnowledgeBaseOwnershipPostDrainAck(
				FORK_KB_OWNERSHIP_POST_DRAIN_ACK,
			),
