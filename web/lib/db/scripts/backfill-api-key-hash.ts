 ...
 *     bun run lib/db/scripts/backfill-api-key-hash.ts
 *   # or
 *   POSTGRES_URL=... API_ENCRYPTION_KEY=... \
 *     bun run lib/db/scripts/backfill-api-key-hash.ts --dry-run
 */
...
function decryptApiKey(
	...
	if (!ivHex || !encrypted || !authTagHex) {
		throw new Error(
			'Invalid encrypted api_key format. Expected "iv:encrypted:authTag"',
		);
	}
...
export function deriveKeyHashForStoredKey(
	...
	if (isEncryptedKey(storedKey)) {
		if (!apiEncryptionKey) {
			throw new Error(
				"API_ENCRYPTION_KEY is required to decrypt an encrypted stored key",
			);
		}
			...
			await Promise.all(
				rows.map(async (row) => {
					...
					try {
						...
						const keyHash = deriveKeyHashForStoredKey(
							row.key,
							apiEncryptionKey,
						);
						...
						stats.failed += 1;
						console.error(
							`Failed to backfill api_key id=${row.id}: ${getErrorMessage(error)}`,
						);
					}
				}),
			);
