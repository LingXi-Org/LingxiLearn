 ...
 *
 * Usage:
 *   cd lib/db && bun run scripts/seed-stress-test-users.ts
 */
...
function randomDate(start: Date, end: Date): string {
	const date = new Date(
		start.getTime() + randomFloat() * (end.getTime() - start.getTime()),
	);
...
async function main() {
	...
	const userResult = (await db.execute(`SELECT id FROM "user" LIMIT 1`)) as {
		id: string;
	}[];
	const userId =
		Array.isArray(userResult) && userResult[0] ? userResult[0].id : "system";
	...
	if (existingTable.length > 0 && existingTable[0].name === TABLE_NAME) {
		...
		console.log(
			`Table ${TABLE_NAME} already exists (${tableId}), clearing existing rows...`,
		);
