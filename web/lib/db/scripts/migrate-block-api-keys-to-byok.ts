...
//   bun run lib/db/scripts/migrate-block-api-keys-to-byok.ts --dry-run \
...
//   bun run lib/db/scripts/migrate-block-api-keys-to-byok.ts \
//     --map jina=jina --map perplexity=perplexity --map google_books=google_cloud \
...
if (
	...
	console.error(
		"No --map or --model-map arguments provided. Specify at least one.",
	);
	console.error(
		"Dedicated blocks: --map jina=jina --map perplexity=perplexity --map google_books=google_cloud",
	);
	console.error(
		`LLM-family blocks (${LLM_FAMILY_BLOCK_TYPES.join("/")}): --model-map grok=xai`,
	);
	process.exit(1);
}
...
const misusedLlmBlockMaps = Object.keys(BLOCK_TYPE_TO_PROVIDER).filter(
	(blockType) =>
		LLM_FAMILY_BLOCK_TYPES.includes(
			blockType as (typeof LLM_FAMILY_BLOCK_TYPES)[number],
		),
);
...
if (!DRY_RUN && !FROM_FILE) {
	console.error(
		"Live runs require --from-file. Run with --dry-run first to generate the file.",
	);
	process.exit(1);
}
if (DRY_RUN && FROM_FILE) {
...
async function processWorkspace(
	...
	try {
		...
		for (const block of blocks) {
			...
			if (
				LLM_FAMILY_BLOCK_TYPES.includes(
					block.blockType as (typeof LLM_FAMILY_BLOCK_TYPES)[number],
				)
			) {
		...
		if (needsEnvVars) {
			...
			const userIds = [
				...new Set([...providerKeys.values()].flat().map((r) => r.userId)),
			];
			if (userIds.length > 0) {
		...
		for (const [providerId, refs] of providerKeys) {
			...
			for (const ref of refs) {
				const { key, source, envVarFailed } = await resolveKey(
					ref,
					envLookup,
					resolveCtx,
				);
			...
			resolved.sort(
				(a, b) => KEY_SOURCE_PRIORITY[a.source] - KEY_SOURCE_PRIORITY[b.source],
			);
			...
			if (distinctKeys.size > 1) {
				stats.conflicts++;
				console.log(
					`  [CONFLICT] provider "${providerId}": ${distinctKeys.size} distinct keys`,
				);
				...
				const chosenIsOwner =
					resolved[0].ref.userId === workspaceOwnerId ? ", owner" : "";
				console.log(
			...
			if (existingBYOKProviders.has(providerId)) {
				console.log(
					`  [SKIP] BYOK already exists for provider "${providerId}"`,
				);
				stats.skippedExisting++;
				continue;
			}
			...
			try {
				...
				console.log(
					`  [INSERT] BYOK for provider "${providerId}": ${maskKey(chosen.key)}`,
				);
				stats.inserted++;
			} catch (error) {
				console.error(
					`  [ERROR] Failed to insert BYOK for provider "${providerId}":`,
					error,
				);
				stats.errors++;
			}
...
async function run() {
	...
	console.log(
		`Users: ${USER_FILTER.length > 0 ? USER_FILTER.join(", ") : "all"}`,
	);
	...
	try {
		...
		const llmFamilyTypes =
			Object.keys(MODEL_PREFIX_TO_PROVIDER).length > 0
				? [...LLM_FAMILY_BLOCK_TYPES]
				: [];
		const allBlockTypes = [
			...new Set([...mappedBlockTypes, ...agentTypes, ...llmFamilyTypes]),
		];
		...
		if (DRY_RUN) {
			...
			console.log(
				`Found ${workspaceIds.length} workspaces with candidate blocks\n`,
			);

			const outPath = resolve("migrate-byok-workspace-ids.txt");
			writeFileSync(outPath, "");
			console.log(
				`[DRY RUN] Will write workspace IDs with keys to ${outPath}\n`,
			);
		} else {
			...
			workspaceIds = raw
				...
				.filter(Boolean);
			console.log(
				`Loaded ${workspaceIds.length} workspace IDs from ${FROM_FILE}\n`,
			);
		}
		...
		for (
			...
			batchStart += WORKSPACE_BATCH_SIZE
		) {
				...
				if (DRY_RUN) {
					const workspaceIdsWithKeys = results
						.map((result, resultIndex) =>
							result.shouldWriteWorkspaceId
								? workspaceChunk[resultIndex]
								: null,
						)
						.filter((id): id is string => id !== null);
		...
		console.log(
			`  BYOK keys skipped (already existed): ${stats.skippedExisting}`,
		);
