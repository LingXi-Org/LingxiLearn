...
if (!url) {
	...
	console.error("Ensure lib/db/.env is configured.");
	process.exit(1);
}
 ...
 * `lib/db/db.ts`. Script migrations bind JS arrays directly through
...
async function connectWithRetry(): Promise<void> {
	for (let attempt = 1; ; attempt++) {
		try {
			...
			if (!isTransientConnectError(error) || attempt >= CONNECT_MAX_ATTEMPTS)
				throw error;
