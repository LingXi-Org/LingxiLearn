 ...
 */
export function getWorkflowBlockParentId(
...
export function isWorkflowBlockProtected(
	...
	return Boolean(
		block.locked || isWorkflowBlockAncestorLocked(blockId, blocks),
	);
}
...
export function filterUniqueWorkflowEdges<T extends WorkflowEdgeHandles>(
	...
	for (const edge of edgesToAdd) {
		...
		if (
			workingEdges.some((existing) => isDuplicateWorkflowEdge(edge, existing))
		)
			continue;
		workingEdges.push(edge);
...
export function getWorkflowEdgeScopeDropReason(
	...
	if (
		targetParent === edge.source &&
		WORKFLOW_CONTAINER_BLOCK_TYPES.has(sourceBlock.type ?? "")
	) {
		return null;
	}

	if (
		sourceParent === edge.target &&
		WORKFLOW_CONTAINER_BLOCK_TYPES.has(targetBlock.type ?? "")
	) {
		return null;
	}
...
export function isWorkflowAnnotationOnlyBlockType(
 ...
 */
export function isKnownWorkflowTriggerBlock(
	block: WorkflowTriggerCapableBlock,
): boolean {
 ...
 */
export const RESERVED_WORKFLOW_BLOCK_NAMES = [
	"loop",
	"parallel",
	"variable",
] as const;
...
export function getWorkflowBlockNameConflict(
	...
	const conflict = Object.entries(siblingNamesById).find(
		([id, siblingName]) =>
			id !== blockId && normalizeWorkflowBlockName(siblingName) === normalized,
	);
	if (conflict) return { reason: "duplicate", conflictingBlockId: conflict[0] };

	if (
		(RESERVED_WORKFLOW_BLOCK_NAMES as readonly string[]).includes(normalized)
	) {
