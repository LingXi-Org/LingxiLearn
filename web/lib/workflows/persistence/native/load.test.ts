...
const tables = vi.hoisted(() => ({
	...
	workflowSubflows: {
		name: "workflowSubflows" as const,
		workflowId: "workflow_id",
	},
}));
	...
	return {
		select: () => ({
			from: (table: { name: string }) =>
				resultFor(rowsByTable[table.name] ?? []),
		}),
	};
}
...
describe("loadWorkflowFromNormalizedTablesRaw retry normalization", () => {
	...
	it("pins a disabled policy without discarding it", async () => {
		expect(
			await loadRetry({ enabled: false, maxTries: 99, waitBetweenTriesMs: -4 }),
		).toEqual({
			enabled: false,
			maxTries: 5,
	...
	it("leaves an in-range policy untouched", async () => {
		expect(
			await loadRetry({ enabled: true, maxTries: 4, waitBetweenTriesMs: 250 }),
		).toEqual({
			enabled: true,
			maxTries: 4,
