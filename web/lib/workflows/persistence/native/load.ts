 ...
 */
function normalizeStoredBlockRetry(
	stored: unknown,
): BlockRetryConfig | undefined {
...
export async function loadWorkflowFromNormalizedTablesRaw(
	...
	try {
		...
		const edgesArray: Edge[] = edges.map((edge) => ({
			...
			sourceHandle:
				normalizeWorkflowEdgeSourceHandle(edge.sourceHandle) ?? undefined,
			targetHandle:
				normalizeWorkflowEdgeTargetHandle(edge.targetHandle) ?? undefined,
			type: "default",
			data: {},
		}));
		...
		subflows.forEach((subflow) => {
			...
			if (subflow.type === SUBFLOW_TYPES.LOOP) {
				...
				const loop: Loop = {
					id: subflow.id,
					nodes: Array.isArray((config as Loop).nodes)
						? (config as Loop).nodes
						: [],
					iterations:
						typeof (config as Loop).iterations === "number"
							? (config as Loop).iterations
							: 1,
					loopType,
				...
				if (blocksMap[subflow.id]) {
					...
					blocksMap[subflow.id] = {
						...
						data: {
							...
							whileCondition:
								loop.whileCondition ?? block.data?.whileCondition ?? "",
							doWhileCondition:
								loop.doWhileCondition ?? block.data?.doWhileCondition ?? "",
						},
					};
				}
				...
				const parallel: Parallel = {
					id: subflow.id,
					nodes: Array.isArray((config as Parallel).nodes)
						? (config as Parallel).nodes
						: [],
					count:
						typeof (config as Parallel).count === "number"
							? (config as Parallel).count
							: 5,
				...
				logger.warn(
					`Unknown subflow type: ${subflow.type} for subflow ${subflow.id}`,
				);
			}
		});
		...
		logger.error(
			`Error loading workflow ${workflowId} from normalized tables:`,
			error,
		);
		return null;
	}
...
export async function persistMigratedBlocks(
	...
	try {
		for (const [blockId, block] of Object.entries(migratedBlocks)) {
			if (block !== originalBlocks[blockId]) {
				...
				if (persisted.length === 0) {
					logger.warn(
						"Skipped persisting block migration (row changed since read or missing)",
						{
							workflowId,
							blockId,
							expectedUpdatedAt,
		...
		logger.warn("Failed to persist block migrations", {
			workflowId,
			error: err,
		});
	}
