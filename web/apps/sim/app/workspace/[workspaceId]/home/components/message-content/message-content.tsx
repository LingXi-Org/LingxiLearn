...
interface ReasoningStepsSegment {
  type: 'reasoning'
  steps: ReasoningStep[]
}
...
type MessageSegment =
  ...
  | AgentGroupSegment
  | OptionsSegment
  | ReasoningStepsSegment
  | StoppedSegment

function appendReasoningStep(segments: MessageSegment[], step: ReasoningStep): void {
  const existing = segments.find(
    (segment): segment is ReasoningStepsSegment => segment.type === 'reasoning'
  )
  if (!existing) {
    segments.push({ type: 'reasoning', steps: [step] })
    return
  }
  const index = existing.steps.findIndex((candidate) => candidate.id === step.id)
  if (index === -1) existing.steps.push(step)
  else existing.steps[index] = step
}
  ...
  return segments
    .map((segment) => {
      ...
      if (segment.type === 'reasoning') {
        return `reasoning:${segment.steps.map((step) => `${step.id}:${step.status}:${step.summary.length}`).join(',')}`
      }
...
function parseBlocksWithSpanTree(blocks: ContentBlock[]): MessageSegment[] {
  ...
  for (let i = 0; i < blocks.length; i++) {
    ...
    // Raw thinking is intentionally absent from the transcript. A backend may
    // opt into the safe, structured summary lane explicitly; arbitrary model
    // reasoning text is still ignored.
    if (block.type === 'thinking' || block.type === 'subagent_thinking') {
      if (block.reasoningStep) appendReasoningStep(segments, block.reasoningStep)
      continue
    }
    ...
    if (block.reasoningStep) {
      appendReasoningStep(segments, block.reasoningStep)
      continue
    }
...
function parseBlocksLegacy(blocks: ContentBlock[]): MessageSegment[] {
  ...
  for (let i = 0; i < blocks.length; i++) {
    ...
    // See the span-tree parser: raw thinking is neither visible nor allowed to
    // influence grouping. Only the explicit safe summary shape is rendered.
    if (block.type === 'thinking' || block.type === 'subagent_thinking') {
      if (block.reasoningStep) appendReasoningStep(segments, block.reasoningStep)
      continue
    }
            ...
            case 'reasoning':
              return (
                <div
                  ...
                  className={isStreaming ? 'animate-stream-fade-in' : undefined}
                >
                  <ReasoningSteps steps={segment.steps} isStreaming={isStreaming} />
                </div>
              )
