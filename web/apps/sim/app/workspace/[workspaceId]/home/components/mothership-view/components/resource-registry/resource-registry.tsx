...
/**
 * Lingxi artifacts are carried through Sim's `file` resource family so they
 * can use the existing resource tabs and content host. They are not ordinary
 * uploaded files, though, so their tab icon should describe the actual
 * learning artifact instead of falling back to the document icon.
 */
function LingxiArtifactTabIcon({
  resource,
  className,
}: {
  ...
  const kind = resource.id.split(":")[2];
  const Icon =
    kind === "lesson-intro"
      ? BookOpen
      : kind === "lecture-deck"
  ...
  return <Icon className={cn(className, "text-[var(--text-icon)]")} />;
}

function IconDropdownItem({
  item,
  icon: Icon,
}: DropdownItemRenderProps & { icon: ElementType }) {
  return (
    <>
...
> = {
  ...
  table: {
    ...
    renderDropdownItem: (props) => (
      <IconDropdownItem {...props} icon={TableIcon} />
    ),
  },
  file: {
    type: "file",
    ...
    renderTabIcon: (resource, className) => {
      if (resource.id.startsWith("lingxi-artifact:")) {
        return (
          <LingxiArtifactTabIcon resource={resource} className={className} />
        );
      }
  ...
  knowledgebase: {
    ...
    renderDropdownItem: (props) => (
      <IconDropdownItem {...props} icon={Database} />
    ),
  },
  folder: {
    type: "folder",
    ...
    renderDropdownItem: (props) => (
      <IconDropdownItem {...props} icon={FolderIcon} />
    ),
  },
  filefolder: {
    type: "filefolder",
  ...
  terminal: {
    ...
    renderDropdownItem: (props) => (
      <IconDropdownItem {...props} icon={TerminalWindow} />
    ),
  },
} as const;
...
export function byResourceMenuOrder<T extends { type: MothershipResourceType }>(
  ...
  return (
    RESOURCE_MENU_ORDER.indexOf(a.type) - RESOURCE_MENU_ORDER.indexOf(b.type)
  );
}
...
> = {
  ...
  filefolder: (qc, wId) => {
    qc.invalidateQueries({
      queryKey: workspaceFileFolderKeys.workspaceLists(wId),
    });
